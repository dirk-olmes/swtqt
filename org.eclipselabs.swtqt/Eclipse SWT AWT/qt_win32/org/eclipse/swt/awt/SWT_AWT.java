...
/**
 * This class provides a bridge between SWT and AWT, so that it is possible to
 * embed AWT components in SWT and vice versa.
 * 
 * @see <a href="http://www.eclipse.org/swt/snippets/#awt">Swing/AWT
 *      snippets</a>
...
public class SWT_AWT {
	...
	/**
	 * The name of the embedded Frame class. The default class name for the
	 * platform will be used if <code>null</code>.
	 */
	 ...
	 * @return a <code>java.awt.Frame</code> the embedded frame or
	 *         <code>null</code>.
	 * 
	...
	/**
	 * Creates a new <code>java.awt.Frame</code>. This frame is the root for the
	 * AWT components that will be embedded within the composite. In order for
	 * the embedding to succeed, the composite must have been created with the
	 * SWT.EMBEDDED style.
	 * <p>
	 * IMPORTANT: As of JDK1.5, the embedded frame does not receive mouse
	 * events. When a lightweight component is added as a child of the embedded
	 * frame, the cursor does not change. In order to work around both these
	 * problems, it is strongly recommended that a heavyweight component such as
	 * <code>java.awt.Panel</code> be added to the frame as the root of all
	 * components.
	 * </p>
	 * 
	 * @param parent
	 ...
	 *            <code>java.awt.Frame</code>
	 * @return a <code>java.awt.Frame</code> to be the parent of the embedded
	 *         AWT components
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_NULL_ARGUMENT - if the parent is null</li>
	 *                <li>ERROR_INVALID_ARGUMENT - if the parent Composite does
	 *                not have the SWT.EMBEDDED style</li>
	 *                </ul>
	 * 
	 * @since 3.0
	 */
	public static Frame new_Frame(final Composite parent) {
		...
		Runnable runnable = new Runnable() {
			public void run() {
				try {
					/*
					 * Some JREs have implemented the embedded frame constructor
					 * to take an integer and other JREs take a long. To handle
					 * this binary incompatibility, use reflection to create the
					 * embedded frame.
					 */
					Class clazz = null;
					try {
					...
					try {
						...
						value = constructor.newInstance(new Object[] { new Integer( /* 64 */handle) });
					} catch (Throwable e1) {
						try {
					...
					/*
					 * This is necessary to make lightweight components directly
					 * added to the frame receive mouse events properly.
					 */
					frame.addNotify();
					 ...
					 * TEMPORARY CODE
					 * 
					 * For some reason, the graphics configuration of the
					 * embedded frame is not initialized properly. This causes
					 * an exception when the depth of the screen is changed.
					 */
					try {
		...
		/*
		 * Generate the appropriate events to activate and deactivate the
		 * embedded frame. This is needed in order to make keyboard focus work
		 * properly for lightweights.
		 */
	...
	/**
	 * Creates a new <code>Shell</code>. This Shell is the root for the SWT
	 * widgets that will be embedded within the AWT canvas.
	 * 
	 * @param display
	 *            the display for the new Shell
	 * @param parent
	 *            the parent <code>java.awt.Canvas</code> of the new Shell
	 ...
	 *                <li>ERROR_NULL_ARGUMENT - if the parent is null</li>
	 *                <li>ERROR_INVALID_ARGUMENT - if the parent's peer is not
	 *                created</li>
	 *                </ul>
	 * 
	 * @since 3.0
	...
	public static Shell new_Shell(final Display display, final Canvas parent) {
		...
		final Shell shell = Shell.qt_new(display);

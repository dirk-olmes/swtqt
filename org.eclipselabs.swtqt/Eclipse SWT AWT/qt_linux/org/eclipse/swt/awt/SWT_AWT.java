...
public class SWT_AWT {
	...
	static synchronized void initializeSwing() {
		...
		/*
		 * Feature in GTK. The default X error handler for GTK calls exit()
		 * after printing the X error. Normally, this isn't that big a problem
		 * for SWT applications because they don't cause X errors. However,
		 * sometimes X errors are generated by AWT that make SWT exit. The fix
		 * is to hide all X errors when AWT is running.
		 */
	...
	public static Frame new_Frame(final Composite parent) {
		...
		//TODO
		int /* long */handle = 0; //parent.embeddedHandle;
		/*
		...
		try {
			...
			value = constructor.newInstance(new Object[] { new Integer(/* 64 */handle), Boolean.TRUE });
		} catch (Throwable e1) {
			try {
					...
					if (window.getParent() == frame) {
						parent.getDisplay().asyncExec(new Runnable() {
							public void run() {
								...
								/* int */long xWindow = OS.gdk_x11_drawable_get_xid(OS.GTK_WIDGET_WINDOW(OS
										.gtk_widget_get_toplevel(shell.handle)));

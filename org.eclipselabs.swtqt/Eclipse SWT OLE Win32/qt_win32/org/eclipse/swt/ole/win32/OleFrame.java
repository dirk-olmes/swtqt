 ...
 * <ul>
 * <li>position and size the ActiveX Control or OLE Document within the
 * application
 ...
 * <li>translate accelerator keystrokes intended for the container's frame
 * </ul>
 * 
 * <dl>
 * <dt><b>Styles</b>
...
final public class OleFrame extends Composite {
	...
	/**
	 * Create an OleFrame child widget using style bits to select a particular
	 * look or set of properties.
	 * 
	 * @param parent
	 *            a composite widget (cannot be null)
	 * @param style
	...
	static int /* long */getMsgProc(int /* long */code, int /* long */wParam, int /* long */lParam) {
		...
		if (code < 0 || (wParam & OS.PM_REMOVE) == 0) {
			return OS.CallNextHookEx(hHook.value, /* 64 */code, wParam, lParam);
		}
		...
		if (OS.WM_KEYFIRST <= message && message <= OS.WM_KEYLAST) {
			if (display != null) {
				...
				while (hwnd != 0) {
					widget = null; //display.findWidget( hwnd );
					if (widget != null) {
						break;
					}
					hwnd = OS.GetParent(hwnd);
				...
				if (widget != null && widget instanceof OleClientSite) {
					...
					if (site.handle == hwnd) {
						...
						/*
						 * Allow activeX control to translate accelerators
						 * except when a menu is active.
						 */
						...
						case OS.WM_SYSKEYDOWN: {
							...
							default: {
								/*
								 * Bug in Windows. The high bit in the result of
								 * MapVirtualKey() on Windows NT is bit 32 while
								 * the high bit on Windows 95 is bit 16. They
								 * should both be bit 32. The fix is to test the
								 * right bit.
								 */
								int mapKey = OS.MapVirtualKey(/* 64 */msg.wParam, 2);
								if (mapKey != 0) {
						...
						/*
						 * Allow OleClientSite to process key events before
						 * activeX control
						 */
						...
						if (consumed) {
							// In order to prevent this message from also being
							// processed
							// by the application, zero out message, wParam and
							// lParam
	...
	private void createCOMInterfaces() {
		...
		iOleInPlaceFrame = new COMObject(new int[] { 2, 0, 0, 1, 1, 1, 1, 1, 2, 2, 3, 1, 1, 1, 2 }) {
			...
			public int /* long */method14(int /* long */[] args) {
				return TranslateAccelerator(args[0], /* 64 */args[1]);
			}
		};
	}
	...
	private int GetBorder(int /* long */lprectBorder) {
		/*
		 * The IOleInPlaceUIWindow::GetBorder function, when called on a
		 * document or frame window object, returns the outer rectangle
		 * (relative to the window) where the object can put toolbars or similar
		 * controls.
		 */
		if (lprectBorder == 0) {
	...
	/**
	 * 
	 * Returns the application menu items that will appear in the Container
	 * location when an OLE Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 * menus but the application is given the opportunity to merge some of its
	 * menus into the menubar. The application is allowed to insert its menus in
	 * three locations: File (far left), Container(middle) and Window (far right
	 * just before Help). The OLE Document retains control of the Edit, Object
	 * and Help menu locations. Note that an application can insert more than
	 * one menu into a single location.
	 * 
	 * @return the application menu items that will appear in the Container
	 *         location when an OLE Document is in-place activated.
	 * 
	 */
	...
	/**
	 * 
	 * Returns the application menu items that will appear in the File location
	 * when an OLE Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 * menus but the application is given the opportunity to merge some of its
	 ...
	 * one menu into a single location.
	 * 
	 * @return the application menu items that will appear in the File location
	 *         when an OLE Document is in-place activated.
	 * 
	 */
	...
	/**
	 * 
	 * Returns the application menu items that will appear in the Window
	 * location when an OLE Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 ...
	 * one menu into a single location.
	 * 
	 * @return the application menu items that will appear in the Window
	 *         location when an OLE Document is in-place activated.
	 * 
	 */
	...
	private int InsertMenus(int /* long */hmenuShared, int /* long */lpMenuWidths) {
		...
		// TODO
		int /* long */hMenu = 0; // menubar.handle;

		// Create a holder for menu information. This will be passed down to
		// the OS and the OS will fill in the requested information for each
		// menu.
		MENUITEMINFO lpmii = new MENUITEMINFO();
		...
		// Loop over all "File-like" menus in the menubar and get information
		// about the
		// item from the OS.
		int fileMenuCount = 0;
		int newindex = 0;
		if (this.fileMenuItems != null) {
			for (int i = 0; i < this.fileMenuItems.length; i++) {
				...
				if (item != null) {
					...
					lpmii.cch = cch; // lpmii.cch gets updated by
					// GetMenuItemInfo to indicate the
					// exact number of characters in name. Reset it to our max
					// size
					// before each call.
		...
		// Loop over all "Container-like" menus in the menubar and get
		...
		if (this.containerMenuItems != null) {
			for (int i = 0; i < this.containerMenuItems.length; i++) {
				...
				if (item != null) {
					...
					lpmii.cch = cch; // lpmii.cch gets updated by
					// GetMenuItemInfo to indicate the
					// exact number of characters in name. Reset it to a large
					// number
					// before each call.
					if (OS.GetMenuItemInfo(hMenu, index, true, lpmii)) {
						if (OS.InsertMenuItem(hmenuShared, newindex, true, lpmii)) {
		...
		// Loop over all "Window-like" menus in the menubar and get information
		// about the
		// item from the OS.
	...
	private int QueryInterface(int /* long */riid, int /* long */ppvObject) {
		// implements IUnknown, IOleInPlaceFrame, IOleContainer,
		// IOleInPlaceUIWindow
	...
	private int SetBorderSpace(int /* long */pborderwidths) {
		...
		// Use no toolbars, but force the container to remove its toolbars by
		// passing a
		// valid BORDERWIDTHS structure containing nothing but zeros in the
		// pborderwidths parameter, or,
		// Use no toolbars but allow the in-place container to leave its
		// toolbars up by
	...
	/**
	 * 
	 * Specify the menu items that should appear in the Container location when
	 * an OLE Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 * menus but the application is given the opportunity to merge some of its
	 ...
	 * 
	 * <p>
	 * This method must be called before in place activation of the OLE
	 * Document. After the Document is activated, the menu bar will not be
	 * modified until a subsequent activation.
	 * 
	 * @param containerMenus
	 *            an array of top level MenuItems to be inserted into the
	 *            Container location of the menubar
	 */
	...
	/**
	 * 
	 * Specify the menu items that should appear in the File location when an
	 * OLE Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 * menus but the application is given the opportunity to merge some of its
	 ...
	 * 
	 * @param fileMenus
	 *            an array of top level MenuItems to be inserted into the File
	...
	private int SetMenu(int /* long */hmenuShared, int /* long */holemenu, int /* long */hwndActiveObject) {
		...
		Menu menubar = getShell().getMenuBar();
		if (menubar == null || menubar.isDisposed()) {
			return COM.OleSetMenuDescriptor(0, getShell().handle, hwndActiveObject, iOleInPlaceFrame.getAddress(),
					inPlaceActiveObject);
		}
		...
		return COM.OleSetMenuDescriptor(holemenu, handle, hwndActiveObject, iOleInPlaceFrame.getAddress(),
				inPlaceActiveObject);
	}

	/**
	 * 
	 * Set the menu items that should appear in the Window location when an OLE
	 * Document is in-place activated.
	 * 
	 * <p>
	 * When an OLE Document is in-place active, the Document provides its own
	 * menus but the application is given the opportunity to merge some of its
	 ...
	 * 
	 * @param windowMenus
	 *            an array of top level MenuItems to be inserted into the Window
	 *            location of the menubar
	 */

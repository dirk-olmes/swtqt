...
/**
 * OleClientSite provides a site to manage an embedded OLE Document within a
 * container.
 * 
 * <p>
 * The OleClientSite provides the following capabilities:
 * <ul>
 * <li>creates the in-place editor for a blank document or opening an existing
 * OLE Document
 ...
 * 
 * <p>
 * This object implements the OLE Interfaces IUnknown, IOleClientSite,
 * IAdviseSink, IOleInPlaceSite
 * 
 * <p>
 ...
 * <dd>BORDER
 * <dt><b>Events</b>
 * <dd>Dispose, Move, Resize
 * </dl>
 * 
 * @see <a href="http://www.eclipse.org/swt/snippets/#ole">OLE and ActiveX
 *      snippets</a>
 * @see <a href="http://www.eclipse.org/swt/examples.php">SWT Examples:
 *      OLEExample, OleWebBrowser</a>
 */
public class OleClientSite extends Composite {
	...
	private int aspect; // the display aspect of the embedded object, e.g.,
	// DvaspectContent or DvaspectIcon
	private int type; // Indicates the type of client that can be supported
		...
		listener = new Listener() {
			public void handleEvent(Event e) {
				...
				case SWT.Traverse:
					onTraverse(e);
					break;
				case SWT.KeyDown: /* required for traversal */
					break;
				default:
	...
	/**
	 * Create an OleClientSite child widget using the OLE Document type
	 * associated with the specified file. The OLE Document type is determined
	 * either through header information in the file or through a Registry entry
	 * for the file extension. Use style bits to select a particular look or set
	 * of properties.
	 * 
	 * @param parent
	 *            a composite widget; must be an OleFrame
	 * @param style
	 ...
	 *                <li>ERROR_INVALID_ARGUMENT when the parent is not an
	 *                OleFrame
	 *                </ul>
	 ...
	 *                thread
	 *                <li>ERROR_CANNOT_CREATE_OBJECT when failed to create OLE
	 *                Object
	 *                <li>ERROR_CANNOT_OPEN_FILE when failed to open file
	 *                <li>ERROR_INTERFACE_NOT_FOUND when unable to create
	 *                callbacks for OLE Interfaces
	...
	public OleClientSite(Composite parent, int style, File file) {
		...
		try {
			...
			result = COM.OleCreateFromFile(appClsid, fileName, COM.IIDIUnknown, COM.OLERENDER_DRAW, null,
					iOleClientSite.getAddress(), tempStorage.getAddress(), address);
			if (result != COM.S_OK) {
	...
	/**
	 * Create an OleClientSite child widget to edit a blank document using the
	 * specified OLE Document application. Use style bits to select a particular
	 * look or set of properties.
	 * 
	 * @param parent
	 *            a composite widget; must be an OleFrame
	 * @param style
	 ...
	 *            the value of the ProgID key or the value of the
	 *            VersionIndependentProgID key specified in the registry for the
	 *            desired OLE Document (for example, the
	 *            VersionIndependentProgID for Word is Word.Document)
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_NULL_ARGUMENT when the parent is null
	 ...
	 *                <li>ERROR_THREAD_INVALID_ACCESS when called from the wrong
	 *                thread
	 *                <li>ERROR_INVALID_CLASSID when the progId does not map to
	 *                a registered CLSID
	 *                <li>ERROR_CANNOT_CREATE_OBJECT when failed to create OLE
	 *                Object
	 *                </ul>
	...
	public OleClientSite(Composite parent, int style, String progId) {
		...
		try {
			...
			// Create ole object with storage object
			int /* long */[] address = new int /* long */[1];
			int result = COM.OleCreate(appClsid, COM.IIDIUnknown, COM.OLERENDER_DRAW, null,
					iOleClientSite.getAddress(), tempStorage.getAddress(), address);
			if (result != COM.S_OK) {
				OLE.error(OLE.ERROR_CANNOT_CREATE_OBJECT, result);
			}
	...
	/**
	 * Create an OleClientSite child widget to edit the specified file using the
	 * specified OLE Document application. Use style bits to select a particular
	 * look or set of properties.
	 * <p>
	 * <b>IMPORTANT:</b> This method is <em>not</em> part of the public API for
	 * <code>OleClientSite</code>. It is marked public only so that it can be
	...
	public OleClientSite(Composite parent, int style, String progId, File file) {
		...
		try {
			...
			if (COM.IsEqualGUID(appClsid, fileClsid)) {
				// Using the same application that created file, therefore, use
				// default mechanism.
				tempStorage = createTempStorage();
				// Create ole object with storage object
				int /* long */[] address = new int /* long */[1];
				int result = COM.OleCreateFromFile(appClsid, fileName, COM.IIDIUnknown, COM.OLERENDER_DRAW, null,
						iOleClientSite.getAddress(), tempStorage.getAddress(), address);
				if (result != COM.S_OK) {
					OLE.error(OLE.ERROR_CANNOT_CREATE_OBJECT, result);
				}
				...
				// Not using the same application that created file, therefore,
				// copy from original file to a new storage file
				IStorage storage = null;
				if (COM.StgIsStorageFile(fileName) == COM.S_OK) {
					int /* long */[] address = new int /* long */[1];
					int mode = COM.STGM_READ | COM.STGM_TRANSACTED | COM.STGM_SHARE_EXCLUSIVE;
					int result = COM.StgOpenStorage(fileName, 0, mode, 0, 0, address); // Does
					// an
					// AddRef
					// if
					// successful
					...
					// Original file is not a Storage file so copy contents to a
					// stream in a new storage file
					int /* long */[] address = new int /* long */[1];
					int mode = COM.STGM_READWRITE | COM.STGM_DIRECT | COM.STGM_SHARE_EXCLUSIVE | COM.STGM_CREATE;
					int result = COM.StgCreateDocfile(null, mode | COM.STGM_DELETEONRELEASE, 0, address); // Increments
					// ref
					// count
					// if
					// successful
					...
					// Word does not follow the standard and does not use
					// "CONTENTS" as the name of
					// its primary stream
					...
					result = storage.CreateStream(streamName, mode, 0, 0, address); // Increments
					// ref
					// count
					// if
					// successful
				...
				result = COM.CoCreateInstance(appClsid, 0, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER,
						COM.IIDIUnknown, ppv);
				if (result != COM.S_OK) {
					OLE.error(OLE.ERROR_CANNOT_CREATE_OBJECT, result);
				}
	...
	protected void addObjectReferences() {
		...
		/*
		 * Feature in Windows. Despite the fact that the clientSite was provided
		 * during the creation of the OleObject (which is required by WMP11 -
		 * see bug 173556), some applications choose to ignore this optional
		 * parameter (see bug 211663) during OleCreate. The fix is to check
		 * whether the clientSite has already been set and set it. Note that
		 * setting it twice can result in assert failures.
		 */
		...
		if (ppvClientSite[0] == 0) {
			...
			Release(); // GetClientSite performs an AddRef so we must release
			// it.
		}
	...
	protected void createCOMInterfaces() {
		...
		iAdviseSink = new COMObject(new int[] { 2, 0, 0, 2, 2, 1, 0, 0 }) {
			...
			public int /* long */method4(int /* long */[] args) {
				return OnViewChange( /* 64 */args[0], /* 64 */args[1]);
			}
		...
		iOleInPlaceSite = new COMObject(
				new int[] { 2, 0, 0, 1, 1, 0, 0, 0, 5, C.PTR_SIZEOF == 4 ? 2 : 1, 1, 0, 0, 0, 1 }) {
			@Override
			public int /* long */method0(int /* long */[] args) {
				return QueryInterface(args[0], args[1]);
			}
			...
			@Override
			public int /* long */method4(int /* long */[] args) {
				return ContextSensitiveHelp( /* 64 */args[0]);
			}
			...
			public int /* long */method9(int /* long */[] args) {
				if (args.length == 2) {
					return Scroll( /* 64 */args[0], /* 64 */args[1]);
				} else {
			...
			public int /* long */method10(int /* long */[] args) {
				return OnUIDeactivate( /* 64 */args[0]);
			}
	...
	/**
	 * Deactivates an active in-place object and discards the object's undo
	 * state.
	 */
	...
	/**
	 * Requests that the OLE Document or ActiveX Control perform an action;
	 * actions are almost always changes to the activation state.
	 * 
	 * @param verb
	 *            the operation that is requested. This is one of the
	 *            OLE.OLEIVERB_ values
	 * 
	 * @return an HRESULT value indicating the success of the operation request;
	 *         OLE.S_OK indicates success
	 */
	public int doVerb(int verb) {
		// Not all OLE clients (for example PowerPoint) can be set into the
		// running state in the constructor.
		// The fix is to ensure that the client is in the running state before
		// invoking any verb on it.
		if (state == STATE_NONE) {
			if (COM.OleRun(objIUnknown.getAddress()) == OLE.S_OK) {
				state = STATE_RUNNING;
			}
	...
	/**
	 * Asks the OLE Document or ActiveX Control to execute a command from a
	 * standard list of commands. The OLE Document or ActiveX Control must
	 * support the IOleCommandTarget interface. The OLE Document or ActiveX
	 * Control does not have to support all the commands in the standard list.
	 * To check if a command is supported, you can call queryStatus with the
	 * cmdID.
	 * 
	 * @param cmdID
	 *            the ID of a command; these are the OLE.OLECMDID_ values - a
	 *            small set of common commands
	 * @param options
	 *            the optional flags; these are the OLE.OLECMDEXECOPT_ values
	 * @param in
	...
	private int GetContainer(int /* long */ppContainer) {
		 ...
		 * probably do not need to implement this method. Instead, they can
		 * return E_NOINTERFACE and set ppContainer to NULL.
		 */
		if (ppContainer != 0) {
	...
	private int GetWindowContext(int /* long */ppFrame, int /* long */ppDoc, int /* long */lprcPosRect,
			int /* long */lprcClipRect, int /* long */lpFrameInfo) {
		...
		if (ppDoc != 0) {
			COM.MoveMemory(ppDoc, new int /* long */[] { 0 }, OS.PTR_SIZEOF);
		}
		...
		if (menubar != null && !menubar.isDisposed()) {
			...
			int cAccel = /* 64 */OS.SendMessage(hwnd, OS.WM_APP, 0, 0);
			if (cAccel != 0) {
	...
	/**
	 * Returns whether ole document is dirty by checking whether the content of
	 * the file representing the document is dirty.
	 * 
	...
	public boolean isDirty() {
		/*
		 * Note: this method must return true unless it is absolutely clear that
		 * the contents of the Ole Document do not differ from the contents in
	...
	private void onDispose(Event e) {
		...
		releaseObjectInterfaces(); // Note, must release object interfaces
		// before releasing frame
		deleteTempStorage();
	...
	/**
	 * Returns the status of the specified command. The status is any bitwise
	 * OR'd combination of SWTOLE.OLECMDF_SUPPORTED, SWTOLE.OLECMDF_ENABLED,
	 * SWTOLE.OLECMDF_LATCHED, SWTOLE.OLECMDF_NINCHED. You can query the status
	 * of a command before invoking it with OleClientSite.exec. The OLE Document
	 * or ActiveX Control must support the IOleCommandTarget to make use of this
	 * method.
	 * 
	 * @param cmd
	 *            the ID of a command; these are the OLE.OLECMDID_ values - a
	 *            small set of common commands
	 * 
	 * @return the status of the specified command or 0 if unable to query the
	 *         OLE Object; these are the OLE.OLECMDF_ values
	 */
	...
	/**
	 * Saves the document to the specified file and includes OLE specific
	 * information if specified. This method must <b>only</b> be used for files
	 * that have an OLE Storage format. For example, a word file edited with
	 * Word.Document should be saved using this method because there is
	 * formating information that should be stored in the OLE specific Storage
	 * format.
	 * 
	 * @param file
	 *            the file to which the changes are to be saved
	 * @param includeOleInfo
	 *            the flag to indicate whether OLE specific information should
	 *            be saved.
	 * 
	...
	private boolean saveFromContents(int /* long */address, File file) {
		...
		try {
			...
			while (tempContents.Read(pv, increment, pcbWritten) == COM.S_OK && pcbWritten[0] > 0) {
				...
				writer.write(buffer); // Note: if file does not exist, this
				// will create the file the
	...
	private boolean saveFromOle10Native(int /* long */address, File file) {
		...
		// The "\1Ole10Native" stream contains a DWORD header whose value is the
		// length
	...
	/**
	 * Saves the document to the specified file and includes OLE specific
	 * information. This method must <b>only</b> be used for files that have an
	 * OLE Storage format. For example, a word file edited with Word.Document
	 * should be saved using this method because there is formating information
	...
	private boolean saveToStorageFile(File file) {
		// The file will be saved using the formating of the current application
		// - this
		// may not be the format of the application that was originally used to
		// create the file
		// e.g. if an Excel file is opened in Word, the Word application will
		// save the file in the
		// Word format
		// Note: if the file already exists, some applications will not
		// overwrite the file
		// In these cases, you should delete the file first (probably save the
		// contents of the file in case the
		// save fails)
		...
		if (objIOleObject.QueryInterface(COM.IIDIPersistFile, address) == COM.S_OK) {
			...
			if (persistFile.GetCurFile(ppszFileName) == COM.S_OK) {
				...
				// Doc says to use IMalloc::Free, but CoTaskMemFree() does the
				// same
		...
		try {
			...
			int result = COM.StgCreateDocfile(path, mode, 0, address); // Does
			// an
			// AddRef
			// if
			// successful
	 ...
	 * files that do not have an OLE Storage format. For example, a bitmap file
	 * edited with MSPaint should be saved using this method because bitmap is a
	 * standard format that does not include any OLE specific data.
	 * 
	 * @param file
	 *            the file to which the changes are to be saved
	 * 
	...
	private boolean saveToTraditionalFile(File file) {
		...
		/*
		 * Bug in Office 2007. Saving Office 2007 documents to compound file
		 * storage object causes the output file to be corrupted. The fix is to
		 * detect Office 2007 documents using the program ID and save only the
		 * content of the 'Package' stream.
		 */
		if (isOffice2007()) {
			/*
			 * Excel fails to open the package stream when the PersistStorage is
			 * not in hands off mode
			 */
			int /* long */[] ppv = new int /* long */[1];
		...
		if (tempStorage.OpenStream("CONTENTS", 0, COM.STGM_DIRECT | COM.STGM_READ | COM.STGM_SHARE_EXCLUSIVE, 0,
				address) == COM.S_OK) {
			return saveFromContents(address[0], file);
		}

		// Look for Ole 1.0 object stream
		if (tempStorage.OpenStream("\1Ole10Native", 0, COM.STGM_DIRECT | COM.STGM_READ | COM.STGM_SHARE_EXCLUSIVE, 0,
				address) == COM.S_OK) {
	...
	void setBounds() {
		...
		setBounds(borderWidths.left, borderWidths.top, area.width - borderWidths.left - borderWidths.right, area.height
	...
	private void setExtent(int width, int height) {
		...
		if (objIOleObject.SetExtent(aspect, newExtent) == COM.S_OK) {
			...
			inUpdate = false;
			if (!alreadyRunning) {
				// Close server if it wasn't already running upon entering this
				// method.
	...
	private int ShowObject() {
		/*
		 * Tells the container to position the object so it is visible to the
		 * user. This method ensures that the container itself is visible and
		 * not minimized.
		 */
		setBounds();
		return COM.S_OK;
	...
	/**
	 * Displays a dialog with the property information for this OLE Object. The
	 * OLE Document or ActiveX Control must support the ISpecifyPropertyPages
	 * interface.
	 * 
	 * @param title
	...
	public void showProperties(String title) {
		...
		result = COM.OleCreatePropertyFrame(frame.handle, 10, 10, chTitle, 1, new int /* long */[] { objIUnknown
				.getAddress() }, caGUID.cElems, caGUID.pElems, COM.LOCALE_USER_DEFAULT, 0, 0);
	...
	private SIZE xFormHimetricToPixels(SIZE aSize) {
		...
		OS.ReleaseDC(0, hDC);
		int cx = Compatibility.round(aSize.cx * xppi, 2540); // 2540 HIMETRIC
		// units per
		// inch
	...
	private SIZE xFormPixelsToHimetric(SIZE aSize) {
		...
		int yppi = OS.GetDeviceCaps(hDC, 90); // logical pixels/inch in y
		OS.ReleaseDC(0, hDC);
		int cx = Compatibility.round(aSize.cx * 2540, xppi); // 2540 HIMETRIC
		// units per
		// inch

...
/**
 * OleControlSite provides a site to manage an embedded ActiveX Control within a
 * container.
 * 
 * <p>
 * In addition to the behaviour provided by OleClientSite, this object provides
 * the following:
 * <ul>
 * <li>events from the ActiveX control
 * <li>notification of property changes from the ActiveX control
 * <li>simplified access to well known properties of the ActiveX Control (e.g.
 * font, background color)
 * <li>expose ambient properties of the container to the ActiveX Control
 * </ul>
 * 
 * <p>
 * This object implements the OLE Interfaces IOleControlSite, IDispatch, and
 * IPropertyNotifySink.
 * 
 * <p>
...
public class OleControlSite extends OleClientSite {
	...
	/**
	 * Create an OleControlSite child widget using the OLE Document type
	...
	/**
	 * Create an OleControlSite child widget using style bits to select a
	 * particular look or set of properties.
	 * 
	 * @param parent
	 *            a composite widget; must be an OleFrame
	 * @param style
	 *            the bitwise OR'ing of widget styles
	 * @param progId
	 *            the unique program identifier which has been registered for
	 *            this ActiveX Control; the value of the ProgID key or the value
	 *            of the VersionIndependentProgID key specified in the registry
	 *            for this Control (for example, the VersionIndependentProgID
	 ...
	 *                <li>ERROR_CANNOT_CREATE_OBJECT when failed to create OLE
	 *                Object
	 *                <li>ERROR_CANNOT_ACCESS_CLASSFACTORY when Class Factory
	 *                could not be found
	 *                <li>ERROR_CANNOT_CREATE_LICENSED_OBJECT when failed to
	 *                create a licensed OLE Object
	 *                </ul>
	 */
	public OleControlSite(Composite parent, int style, String progId) {
		...
		try {
			...
			if (licinfo == 0) {
				...
				int result = COM.OleCreate(appClsid, COM.IIDIUnknown, COM.OLERENDER_DRAW, null, iOleClientSite
						.getAddress(), tempStorage.getAddress(), address);
				if (result != COM.S_OK)
				...
				try {
					int result = COM.CoGetClassObject(appClsid, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER,
							0, COM.IIDIClassFactory2, ppvObject);
					if (result != COM.S_OK) {
	 ...
	 * 
	 * @param automation
	 *            the automation object that provides the event notification
	 * @param eventID
	 *            the id of the event
	 * @param listener
	 *            the listener
	...
	protected void createCOMInterfaces() {
		...
		iDispatch = new COMObject(new int[] { 2, 0, 0, 1, 3, 5, 8 }) {
			...
			public int /* long */method6(int /* long */[] args) {
				return Invoke((int) /* 64 */args[0], args[1], (int) /* 64 */args[2], (int) /* 64 */args[3], args[4],
	...
	public Color getBackground() {
		...
		if (objIUnknown != null) {
			// !! We are getting the OLE_COLOR - should we change this to the
			// COLORREF value?
			...
			if (varBackColor != null) {
				...
				// if ( COM.OleTranslateColor( varBackColor.getInt(),
				// getDisplay().hPalette, colorRef ) == COM.S_OK )
				// return Color.qt_new( getDisplay(), colorRef[0] );
			}
		}
	...
	public Font getFont() {
		...
		if (objIUnknown != null) {
			...
			if (varDispFont != null) {
				...
				// Variant lfCharSet =
				// iDispFont.getProperty(COM.DISPID_FONT_CHARSET);
	...
	public Color getForeground() {
		...
		if (objIUnknown != null) {
			...
			if (varForeColor != null) {
				int[] colorRef = new int[1];
				// if (COM.OleTranslateColor(varForeColor.getInt(),
				// getDisplay().hPalette, colorRef) == COM.S_OK)
				// return Color.qt_new(getDisplay(), colorRef[0]);
			}
		}
	...
	protected int /* long */getLicenseInfo(GUID clsid) {
		int /* long */[] ppvObject = new int /* long */[1];
		if (COM.CoGetClassObject(clsid, COM.CLSCTX_INPROC_HANDLER | COM.CLSCTX_INPROC_SERVER, 0, COM.IIDIClassFactory,
				ppvObject) != COM.S_OK) {
			return 0;
		}
		int /* long */result = 0;
	...
	private int Invoke(int dispIdMember, int /* long */riid, int lcid, int dwFlags, int /* long */pDispParams,
			int /* long */pVarResult, int /* long */pExcepInfo, int /* long */pArgErr) {
		if (pVarResult == 0 || dwFlags != COM.DISPATCH_PROPERTYGET) {
			if (pExcepInfo != 0)
				COM.MoveMemory(pExcepInfo, new int /* long */[] { 0 }, OS.PTR_SIZEOF);
			if (pArgErr != 0)
		...
		case COM.DISPID_AMBIENT_SHOWHATCHING:
			if (pVarResult != 0)
				COM.MoveMemory(pVarResult, new int /* long */[] { 0 }, OS.PTR_SIZEOF);
			if (pExcepInfo != 0)
				COM.MoveMemory(pExcepInfo, new int /* long */[] { 0 }, OS.PTR_SIZEOF);
			if (pArgErr != 0)
				COM.MoveMemory(pArgErr, new int[] { 0 }, 4);
	 ...
	 * 
	 * @since 2.0
	 * @deprecated - use OleControlSite.removeEventListener(OleAutomation, int,
	 *             OleListener)
	 * 
	 * @param automation
	 *            the automation object that provides the event notification
	 * 
	 * @param guid
	 *            the identifier of the events COM interface
	 * 
	 * @param eventID
	 *            the event identifier
	 * 
				...
				if (iunknown == oleEventSinkIUnknown[i]) {
					...
					if (!oleEventSink[i].hasListeners()) {
						...
						if (oldLength == 1) {
							...
							System
									.arraycopy(oleEventSinkIUnknown, i + 1, newOleEventSinkIUnknown, i, oldLength - i
											- 1);
							oleEventSinkIUnknown = newOleEventSinkIUnknown;
						}
					}
	...
	public void setBackground(Color color) {
		...
		if (objIUnknown != null) {
			OleAutomation oleObject = new OleAutomation(this);
			oleObject.setProperty(COM.DISPID_BACKCOLOR, new Variant(color.getPixel()));
			oleObject.dispose();
		}
	}
	...
	public void setFont(Font font) {
		...
		if (objIUnknown != null) {
			...
			if (varDispFont != null) {
				...
				// iDispFont.setProperty(COM.DISPID_FONT_CHARSET, new
				// Variant(fdata[0].getCharset));
	...
	public void setForeground(Color color) {
		...
		if (objIUnknown != null) {
			OleAutomation oleObject = new OleAutomation(this);
			oleObject.setProperty(COM.DISPID_FORECOLOR, new Variant(color.getPixel()));
			oleObject.dispose();
		}
	}

	/**
	 * Sets the control site property specified by the dispIdMember to a new
	 * value. The value will be disposed by the control site when it is no
	 * longer required using Variant.dispose. Passing a value of null will clear
	 * the dispId value.
	 * 
	 * @param dispId

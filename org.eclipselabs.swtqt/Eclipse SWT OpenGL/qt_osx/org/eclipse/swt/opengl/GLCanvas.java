 ...
 * 
 * @see GLData
 * @see <a href="http://www.eclipse.org/swt/snippets/#opengl">OpenGL
 *      snippets</a>
...
public class GLCanvas extends Canvas {
	...
	public GLCanvas(Composite parent, int style, GLData data) {
		...
		//TODO NSNotificationCenter.defaultCenter().addObserver(view,  OS.sel_updateOpenGLContext_, OS.NSViewGlobalFrameDidChangeNotification, view);
		...
		Listener listener = new Listener() {
			public void handleEvent(Event event) {
				...
				case SWT.Dispose:
					...
					//TODO NSNotificationCenter.defaultCenter().removeObserver(view);
	...
	public GLData getGLData() {
		...
		/*
		 * Feature in Cocoa: NSOpenGL/CoreOpenGL only supports specifying the
		 * total number of bits in the size of the color component. For
		 * compatibility we split the color size less any alpha into thirds and
		 * allocate a third to each color.
		 */
		pixelFormat.getValues(value, OS.NSOpenGLPFAColorSize, 0);

		int colorSize = (value[0] - data.alphaSize) / 3;
		...
		/*
		 * Feature(?) in Cocoa: NSOpenGL/CoreOpenGL doesn't support setting an
		 * accumulation buffer alpha, but has an alpha if the color values for
		 * the accumulation buffer were set. Allocate the values evenly in that
		 * case.
		 */
		pixelFormat.getValues(value, OS.NSOpenGLPFAAccumSize, 0);

		int accumColorSize = value[0] / 4;
	...
	/**
	 * Returns a boolean indicating whether the receiver's OpenGL context is the
	 * current context.
	 * 
	...
	/**
	 * Sets the OpenGL context associated with this GLCanvas to be the current
	 * GL context.
	 * 
	 * @exception SWTException
	 *                <ul>

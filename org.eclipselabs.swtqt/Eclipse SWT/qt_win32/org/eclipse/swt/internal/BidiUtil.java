...
public class BidiUtil {
	...
	/*
	 * This code is intentionally commented. In order to support CLDC, .class
	 * cannot be used because it does not compile on some Java compilers when
	...
	/*
	 * Public character class constants are the same as Windows platform
	 * constants. Saves conversion of class array in getRenderInfo to arbitrary
	 * constants for now.
	 */
	...
	/**
	 * Adds a language listener. The listener will get notified when the
	 * language of the keyboard changes (via Alt-Shift on Win platforms). Do
	 * this by creating a window proc for the Control so that the window
	 * messages for the Control can be monitored.
	 * <p>
	 * 
	 * @param hwnd
	 *            the handle of the Control that is listening for keyboard
	 *            language changes
	 * @param runnable
	 *            the code that should be executed when a keyboard language
	 *            change occurs
	 */
	...
	/**
	 * Proc used for OS.EnumSystemLanguageGroups call during isBidiPlatform
	 * test.
	 */
	static int /* long */EnumSystemLanguageGroupsProc(int /* long */lpLangGrpId, int /* long */lpLangGrpIdString,
			int /* long */lpLangGrpName, int /* long */options, int /* long */lParam) {
		if (/* 64 */lpLangGrpId == OS.LGRPID_HEBREW) {
			isBidiPlatform = 1;
			return 0;
		}
		if (/* 64 */lpLangGrpId == OS.LGRPID_ARABIC) {
			isBidiPlatform = 1;
			return 0;
		}
		return 1;
	 ...
	 * 
	 * @param gc
	 *            the gc to use for rendering
	 * @param renderBuffer
	 *            the glyphs to render as an array of characters
	 * @param renderDx
	 *            the width of each glyph in renderBuffer
	 * @param x
	...
	/**
	 * Return ordering and rendering information for the given text. Wraps the
	 * GetFontLanguageInfo and GetCharacterPlacement functions.
	 * <p>
	 * 
	 * @param gc
	 *            the GC to use for measuring of this line, input parameter
	 * @param text
	 *            text that bidi data should be calculated for, input parameter
	 * @param order
	 *            an array of integers representing the visual position of each
	 *            character in the text array, output parameter
	 * @param classBuffer
	 *            an array of integers representing the type (e.g., ARABIC,
	 *            HEBREW, LOCALNUMBER) of each character in the text array,
	 *            input/output parameter
	 * @param dx
	 *            an array of integers representing the pixel width of each
	 *            glyph in the returned glyph buffer, output parameter
	 * @param flags
	 *            an integer representing rendering flag information, input
	 *            parameter
	 * @param offsets
	 *            text segments that should be measured and reordered
	 *            separately, input parameter. See
	 *            org.eclipse.swt.custom.BidiSegmentEvent for details.
	...
	/**
	 * Return bidi ordering information for the given text. Does not return
	 * rendering information (e.g., glyphs, glyph distances). Use this method
	 * when you only need ordering information. Doing so will improve
	 * performance. Wraps the GetFontLanguageInfo and GetCharacterPlacement
	 * functions.
	 * <p>
	 * 
	 * @param gc
	 ...
	 * @param gc
	 *            the gc to query
	 * @return bitwise OR of the REORDER, LIGATE and GLYPHSHAPE flags defined by
	 *         this class.
	 */
	 ...
	 * <p>
	 * 
	 * @return an integer representing the active keyboard language
	...
	/**
	 * Return whether or not the platform supports a bidi language. Determine
	 * this by looking at the languages that are installed.
	 * <p>
	 * 
	 * @return true if bidi is supported, false otherwise. Always false on
	 *         Windows CE.
	 */
	public static boolean isBidiPlatform() {
		if (isBidiPlatform != -1) {
			return isBidiPlatform == 1; // already set
		}
	 ...
	 * Determine this by looking at the languages that are installed for the
	 * keyboard.
	 * <p>
	 * 
	...
	/**
	 * Switch the keyboard language to the specified language type. We do not
	 * distinguish between multiple bidi or multiple non-bidi languages, so set
	 * the keyboard to the first language of the given type.
	 * <p>
	 * 
	 * @param language
	 *            integer representing language. One of KEYBOARD_BIDI,
	 *            KEYBOARD_NON_BIDI.
	 */
	 ...
	 * 
	 * @param hwnd
	 *            the handle of the Control to change the orientation of
	 * @param orientation
	 *            one of SWT.RIGHT_TO_LEFT or SWT.LEFT_TO_RIGHT
	...
	public static boolean setOrientation(Control control, int orientation) {
		//TODO
		return false; //setOrientation(control.handle, orientation);
	}
	 ...
	 * 
	 * @param hwnd
	 *            control to override the window proc of
	 */
	 ...
	 * Also reverse the order array if the orientation is to the right.
	 * 
	 * @param orderArray
	 *            integer array of order values to translate
	 * @param glyphCount
	 *            number of glyphs that have been processed for the current line
	 * @param isRightOriented
	 *            flag indicating whether or not current orientation is to the
	 *            right
	 */
	 ...
	 * 
	 * @param hwnd
	 *            control to remove the window proc override for
	 */
	...
	/**
	 * Window proc to intercept keyboard language switch event
	 * (WS_INPUTLANGCHANGE) and widget orientation changes. Run the Control's
	 * registered runnable when the keyboard language is switched.
	 * 
	 * @param hwnd
	 *            handle of the control that is listening for the keyboard
	...
	static int /* long */windowProc(int /* long */hwnd, int /* long */msg, int /* long */wParam, int /* long */lParam) {
		...
		return OS.CallWindowProc(oldProc.value, hwnd, /* 64 */msg, wParam, lParam);
	}

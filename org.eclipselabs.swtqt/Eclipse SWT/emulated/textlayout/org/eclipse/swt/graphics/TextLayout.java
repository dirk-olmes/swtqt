...
public final class TextLayout extends Resource {
	...
	static class StyleItem {
		...
		public String toString() {
			return "StyleItem {" + start + ", " + style + "}"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
	}
		...
		for (int i = 0; i < allRuns.length - 1; i++) {
			...
			if (wrapWidth != -1 && lineWidth + run.width > wrapWidth && !run.tab) {
				...
				while (i >= lineStart) {
					...
					text.getChars(run.start, run.start + run.length, chars, 0);
					while (start >= 0) {
						if (Compatibility.isSpaceChar(chars[start]) || Compatibility.isWhitespace(chars[start])) {
							break;
						}
						start--;
					}
				...
				while (start < run.length) {
					if (!Compatibility.isWhitespace(chars[start])) {
						break;
					}
					start++;
				}
						...
						if (fullSelection) {
							...
							if (!run.tab && !(run.style != null && run.style.metrics != null)) {
								...
								if (run.style != null && run.style.strikeout) {
									int strikeoutY = drawRunY + run.ascent + run.descent - (run.ascent + run.descent)
											/ 2 - 1;
							...
							if (!run.tab) {
								...
								if (!(run.style != null && run.style.metrics != null)) {
									...
									if (run.style != null && run.style.strikeout) {
										int strikeoutY = drawRunY + run.ascent + run.descent
												- (run.ascent + run.descent) / 2 - 1;
										gc.drawLine(drawX, strikeoutY, drawX + run.width, strikeoutY);
									}
								}
	...
	public int getNextOffset(int offset, int movement) {
		...
		if ((movement & (SWT.MOVEMENT_CHAR | SWT.MOVEMENT_CLUSTER)) != 0) {
			return offset + 1;
		}
		int lineEnd = 0;
		...
		while (offset < lineEnd) {
			...
			if (!spaceChar && previousSpaceChar) {
				break;
			}
	 ...
	 * layout has been disposed, it is an error to invoke any other method using
	 * the text layout.
	 * </p>
	 * 
	...
	/**
	 * Sets the indent of the receiver. This indent it applied of the first line
	 * of each paragraph.
	 * 
	 * @param indent
	 *            new indent
	 ...
	 * to override the default behaviour of the bidirectional algorithm.

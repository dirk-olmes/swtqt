...
/**
 * Instances of this class are device-independent descriptions of images. They
 * are typically used as an intermediate format between loading from or writing
 * to streams and creating an <code>Image</code>.
 * <p>
 * Note that the public fields <code>x</code>, <code>y</code>,
 * <code>disposalMethod</code> and <code>delayTime</code> are typically only
 * used when the image is in a set of images used for animation.
 * </p>
 * 
 * @see Image
 * @see ImageLoader
 * @see <a href="http://www.eclipse.org/swt/snippets/#image">ImageData
 *      snippets</a>
...
public final class ImageData implements CloneableCompatibility {
	 ...
	 * the image is direct color. Check the associated PaletteData's isDirect
	 ...
	 * <p>
	 * If one scanline of the image is not a multiple of this number, it will be
	 ...
	 * <p>
	 * Note that for 16 bit depth images the pixel data is stored in least
	 * significant byte order; however, for 24bit and 32bit depth images the
	 * pixel data is stored in most significant byte order.
	 * </p>
	 */
	public byte[] data;
	 ...
	 * <p>
	 * This is a 1 bit bitmap stored with the most significant bit first. The
	 * number of bytes per scanline is '((width + 7) / 8 + (maskPad - 1)) /
	 * maskPad * maskPad'.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * If one scanline of the transparency mask is not a multiple of this
	 * number, it will be padded with zeros until it is.
	 * </p>
	 */
	public int maskPad;
	 ...
	 * <p>
	 * Every pixel can have an <em>alpha blending</em> value that varies from 0,
	 * meaning fully transparent, to 255 meaning fully opaque. The number of
	 * bytes per scanline is 'width'.
	 * </p>
	 */
	 ...
	 * <p>
	 * If this value is set, the <code>alphaData</code> field is ignored and
	 * when the image is rendered each pixel will be blended with the background
	 * an amount proportional to this value.
	 * </p>
	 * <p>
	...
	/**
	 * The x coordinate of the top left corner of the image within the logical
	 * screen (this field corresponds to the GIF89a Image Left Position value).
	 */
	public int x;

	/**
	 * The y coordinate of the top left corner of the image within the logical
	 * screen (this field corresponds to the GIF89a Image Top Position value).
	 */
	public int y;

	/**
	 * A description of how to dispose of the current image before displaying
	 * the next.
	 * 
	 * It is expressed as one of the following values:
	 * <dl>
	...
	/**
	 * The time to delay before displaying the next image in an animation (this
	 * field corresponds to the GIF89a Delay Time value).
	 */
	public int delayTime;

	public List<Integer> colorTable;
	...
	/**
	 * Constructs a new, empty ImageData with the given width, height, depth and
	 * palette. The data will be initialized to an (all zero) array of the
	 * appropriate size.
	 * 
	 * @param width
	 ...
	 * @param palette
	 *            the palette of the image (must not be null)
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if the width or height is
	 *                zero or negative, or if the depth is not one of 1, 2, 4,
	 *                8, 16, 24 or 32</li>
	...
	/**
	 * Constructs a new, empty ImageData with the given width, height, depth,
	 * palette, scanlinePad and data.
	 * 
	 * @param width
	 *            the width of the image
	 * @param height
	 ...
	 * @param scanlinePad
	 *            the padding of each line, in bytes
	 * @param data
	 ...
	 *                <li>ERROR_INVALID_ARGUMENT - if the width or height is
	 *                zero or negative, or if the depth is not one of 1, 2, 4,
	 *                8, 16, 24 or 32, or the data array is too small to contain
	 *                the image data</li>
	...
	public ImageData(int width, int height, int depth, PaletteData palette, int scanlinePad, byte[] data) {
		this(width, height, depth, palette, scanlinePad, checkData(data), 0, null, null, -1, -1, SWT.IMAGE_UNDEFINED,
				0, 0, 0, 0);
	}

	/**
	 * Constructs an <code>ImageData</code> loaded from the specified input
	 * stream. Throws an error if an error occurs while loading the image, or if
	 * the image has an unsupported type. Application code is still responsible
	 ...
	 * 
	 * @param stream
	 *            the input stream to load the image from (must not be null)
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	...
	public ImageData(InputStream stream) {
		...
		ImageData i = data[0];
		setAllFields(i.width, i.height, i.depth, i.scanlinePad, i.bytesPerLine, i.data, i.palette, i.transparentPixel,
				i.maskData, i.maskPad, i.alphaData, i.alpha, i.type, i.x, i.y, i.disposalMethod, i.delayTime);
	}

	/**
	 * Constructs an <code>ImageData</code> loaded from a file with the
	 * specified name. Throws an error if an error occurs loading the image, or
	 * if the image has an unsupported type.
	 * <p>
	 * This constructor is provided for convenience when loading a single image
	 * only. If the file contains multiple images, only the first one will be
	 * loaded. To load multiple images, use <code>ImageLoader.load()</code>.
	 * </p>
	 * 
	 * @param filename
	 *            the name of the file to load the image from (must not be null)
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	 ...
	 *                <li>ERROR_IO - if an IO error occurs while reading from
	 *                the file</li>
	 *                <li>ERROR_INVALID_IMAGE - if the image file contains
	 *                invalid data</li>
	 *                <li>ERROR_UNSUPPORTED_FORMAT - if the image file contains
	 *                an unrecognized format</li>
	 *                </ul>
	 */
	 ...
	 * </p>
	 */
	ImageData(int width, int height, int depth, PaletteData palette, int scanlinePad, byte[] data, int maskPad,
			byte[] maskData, byte[] alphaData, int alpha, int transparentPixel, int type, int x, int y,
			int disposalMethod, int delayTime) {
		...
		/*
		 * When the image is being loaded from a PNG, we need to use the
		 * theoretical minimum number of bytes per line to check whether there
		 * is enough data, because the actual number of bytes per line is
		 * calculated based on the given depth, which may be larger than the
		 * actual depth of the PNG.
		 */
		int minBytesPerLine = type == SWT.IMAGE_PNG ? ((width + 7) / 8 + 3) / 4 * 4 : bytesPerLine;
		if (data != null && data.length < minBytesPerLine * height) {
			SWT.error(SWT.ERROR_INVALID_ARGUMENT);
		}
		setAllFields(width, height, depth, scanlinePad, bytesPerLine, data != null ? data : new byte[bytesPerLine
				* height], palette, transparentPixel, maskData, maskPad, alphaData, alpha, type, x, y, disposalMethod,
				delayTime);
	}

	/**
	 * Initializes all fields in the receiver. This method must be called by all
	 * public constructors to ensure that all fields are initialized for a new
	 * ImageData object. If a new field is added to the class, then it must be
	 * added to this method.
	 * <p>
	 * This method is for internal use, and is not described further.
	 * </p>
	 */
	void setAllFields(int width, int height, int depth, int scanlinePad, int bytesPerLine, byte[] data,
			PaletteData palette, int transparentPixel, byte[] maskData, int maskPad, byte[] alphaData, int alpha,
	...
	/**
	 * Invokes internal SWT functionality to create a new instance of this
	 * class.
	 * <p>
	 * <b>IMPORTANT:</b> This method is <em>not</em> part of the public API for
	 * <code>ImageData</code>. It is marked public only so that it can be shared
	 * within the packages provided by SWT. It is subject to change without
	 * notice, and should never be called from application code.
	 * </p>
	 * <p>
	 * This method is for internal use, and is not described further.
	 * </p>
	 */
	public static ImageData internal_new(int width, int height, int depth, PaletteData palette, int scanlinePad,
			byte[] data, int maskPad, byte[] maskData, byte[] alphaData, int alpha, int transparentPixel, int type,
			int x, int y, int disposalMethod, int delayTime) {
		return new ImageData(width, height, depth, palette, scanlinePad, data, maskPad, maskData, alphaData, alpha,
				transparentPixel, type, x, y, disposalMethod, delayTime);
	}

	ImageData colorMaskImage(int pixel) {
		ImageData mask = new ImageData(width, height, 1, bwPalette(), 2, null, 0, null, null, -1, -1,
				SWT.IMAGE_UNDEFINED, 0, 0, 0, 0);
	 ...
	 * have been filled in with <em>copies</em> of the values in the slots of
	 * the receiver. That is, the returned object is a <em>deep copy</em> of the
	 * receiver.
	 * 
	...
	public Object clone() {
		...
		return new ImageData(width, height, depth, palette, scanlinePad, cloneData, maskPad, cloneMaskData,
				cloneAlphaData, alpha, transparentPixel, type, x, y, disposalMethod, delayTime);
	}

	/**
	 * Returns the alpha value at offset <code>x</code> in scanline
	 * <code>y</code> in the receiver's alpha data. The alpha value is between 0
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if either argument is out of
	 *                range</li>
	 *                </ul>
	 */
	 ...
	 * <code>x</code> in scanline <code>y</code> in the receiver's alpha data
	 * starting at <code>startIndex</code>. The alpha values are unsigned,
	 * between <code>(byte)0</code> (transparent) and <code>(byte)255</code>
	 * (opaque).
	 * 
	 * @param x
	...
	/**
	 * Returns the pixel value at offset <code>x</code> in scanline
	 * <code>y</code> in the receiver's data.
	 * 
	 * @param x
	 ...
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_UNSUPPORTED_DEPTH if the depth is not one of 1,
	 *                2, 4, 8, 16, 24 or 32</li>
	 *                </ul>
	 */
	 ...
	 * <code>x</code> in scanline <code>y</code> in the receiver's data starting
	 ...
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_UNSUPPORTED_DEPTH - if the depth is not one of
	 *                1, 2, 4 or 8 (For higher depths, use the int[] version of
	 *                this method.)</li>
	 *                </ul>
	 */
	...
	/**
	 * Returns an array of <code>RGB</code>s which comprise the indexed color
	 * table of the receiver, or null if the receiver has a direct color model.
	 * 
	...
	/**
	 * Returns an <code>ImageData</code> which specifies the transparency mask
	 * information for the receiver. If the receiver has no transparency or is
	 * not an icon, returns an opaque mask.
	 * 
	 ...
	 * <code>SWT.TRANSPARENCY_PIXEL</code> or
	...
	/**
	 * Returns a copy of the receiver which has been stretched or shrunk to the
	 * specified size. If either the width or height is negative, the resulting
	 * image will be inverted in the associated axis.
	 * 
	 * @param width
	...
	public ImageData scaledTo(int width, int height) {
		...
		ImageData dest = new ImageData(width, height, depth, palette, scanlinePad, null, 0, null, null, -1,
				transparentPixel, type, x, y, disposalMethod, delayTime);

		/* Scale the image contents */
		if (palette.isDirect) {
			blit(BLIT_SRC, this.data, this.depth, this.bytesPerLine, this.getByteOrder(), 0, 0, this.width,
					this.height, 0, 0, 0, ALPHA_OPAQUE, null, 0, 0, 0, dest.data, dest.depth, dest.bytesPerLine, dest
							.getByteOrder(), 0, 0, dest.width, dest.height, 0, 0, 0, flipX, flipY);
		} else {
			blit(BLIT_SRC, this.data, this.depth, this.bytesPerLine, this.getByteOrder(), 0, 0, this.width,
					this.height, null, null, null, ALPHA_OPAQUE, null, 0, 0, 0, dest.data, dest.depth,
					dest.bytesPerLine, dest.getByteOrder(), 0, 0, dest.width, dest.height, null, null, null, flipX,
					flipY);
		}
		...
		if (maskData != null) {
			...
			blit(BLIT_SRC, this.maskData, 1, srcBpl, MSB_FIRST, 0, 0, this.width, this.height, null, null, null,
					ALPHA_OPAQUE, null, 0, 0, 0, dest.maskData, 1, destBpl, MSB_FIRST, 0, 0, dest.width, dest.height,
					null, null, null, flipX, flipY);
		} else if (alpha != -1) {
			dest.alpha = this.alpha;
		} else if (alphaData != null) {
			dest.alphaData = new byte[dest.width * dest.height];
			blit(BLIT_SRC, this.alphaData, 8, this.width, MSB_FIRST, 0, 0, this.width, this.height, null, null, null,
					ALPHA_OPAQUE, null, 0, 0, 0, dest.alphaData, 8, dest.width, MSB_FIRST, 0, 0, dest.width,
					dest.height, null, null, null, flipX, flipY);
		}
		return dest;
	}

	/**
	 * Sets the alpha value at offset <code>x</code> in scanline <code>y</code>
	 * in the receiver's alpha data. The alpha value must be between 0
	 * (transparent) and 255 (opaque).
	 * 
	 * @param x
	...
	/**
	 * Sets the alpha values starting at offset <code>x</code> in scanline
	 * <code>y</code> in the receiver's alpha data to the values from the array
	 * <code>alphas</code> starting at <code>startIndex</code>. The alpha values
	 * must be between <code>(byte)0</code> (transparent) and
	 * <code>(byte)255</code> (opaque)
	 * 
	 * @param x
	 *            the x coordinate of the pixel to being setting the alpha
	 *            values
	 * @param y
	 *            the y coordinate of the pixel to being setting the alpha
	 *            values
	 * @param putWidth
	...
	/**
	 * Sets the pixel value at offset <code>x</code> in scanline <code>y</code>
	...
	/**
	 * Sets the pixel values starting at offset <code>x</code> in scanline
	 * <code>y</code> in the receiver's data to the values from the array
	 ...
	 *                <ul>
	 *                <li>ERROR_UNSUPPORTED_DEPTH if the depth is not one of 1,
	 *                2, 4, 8 (For higher depths, use the int[] version of this
	 *                method.)</li>
	 *                </ul>
	 */
	 ...
	 */
	static int closestMatch(int depth, byte red, byte green, byte blue, int redMask, int greenMask, int blueMask,
		...
		if (depth > 8) {
			...
			return red << 24 >>> rshift & redMask | green << 24 >>> gshift & greenMask | blue << 24 >>> bshift
					& blueMask;
		}
		int r, g, b;
	...
	/**
	 * Blit operation bits to be OR'ed together to specify the desired
	 * operation.
	 */
	...
	// direct / true color formats with arbitrary masks & shifts
			TYPE_GENERIC_8 = 0,
			TYPE_GENERIC_16_MSB = 1, TYPE_GENERIC_16_LSB = 2, TYPE_GENERIC_24 = 3,
	 ...
	 * <p>
	 * Note: When the source and destination depth, order and masks are pairwise
	 * equal and the blitter operation is BLIT_SRC, the masks are ignored. Hence
	 * when not changing the image data format, 0 may be specified for the
	 * masks.
	 * </p>
	 * 
	 * @param op
	 *            the blitter operation: a combination of BLIT_xxx flags (see
	 *            BLIT_xxx constants)
	 * @param srcData
	 *            the source byte array containing image data
	 * @param srcDepth
	 *            the source depth: one of 8, 16, 24, 32
	 * @param srcStride
	 *            the source number of bytes per line
	 * @param srcOrder
	 *            the source byte ordering: one of MSB_FIRST or LSB_FIRST;
	 *            ignored if srcDepth is not 16 or 32
	 * @param srcX
	 *            the top-left x-coord of the source blit region
	 * @param srcY
	 *            the top-left y-coord of the source blit region
	 * @param srcWidth
	 *            the width of the source blit region
	 * @param srcHeight
	 *            the height of the source blit region
	 * @param srcRedMask
	 *            the source red channel mask
	 * @param srcGreenMask
	 *            the source green channel mask
	 * @param srcBlueMask
	 *            the source blue channel mask
	 * @param alphaMode
	 *            the alpha blending or mask mode, may be an integer 0-255 for
	 *            global alpha; ignored if BLIT_ALPHA not specified in the
	 *            blitter operations (see ALPHA_MODE_xxx constants)
	 * @param alphaData
	 *            the alpha blending or mask data, varies depending on the value
	 *            of alphaMode and sometimes ignored
	 * @param alphaStride
	 *            the alpha data number of bytes per line
	 * @param alphaX
	 *            the top-left x-coord of the alpha blit region
	 * @param alphaY
	 *            the top-left y-coord of the alpha blit region
	 * @param destData
	 *            the destination byte array containing image data
	 * @param destDepth
	 *            the destination depth: one of 8, 16, 24, 32
	 * @param destStride
	 *            the destination number of bytes per line
	 * @param destOrder
	 *            the destination byte ordering: one of MSB_FIRST or LSB_FIRST;
	 *            ignored if destDepth is not 16 or 32
	 * @param destX
	 *            the top-left x-coord of the destination blit region
	 * @param destY
	 *            the top-left y-coord of the destination blit region
	 * @param destWidth
	 *            the width of the destination blit region
	 * @param destHeight
	 *            the height of the destination blit region
	 * @param destRedMask
	 *            the destination red channel mask
	 * @param destGreenMask
	 *            the destination green channel mask
	 * @param destBlueMask
	 *            the destination blue channel mask
	 * @param flipX
	 *            if true the resulting image is flipped along the vertical axis
	 * @param flipY
	 *            if true the resulting image is flipped along the horizontal
	 *            axis
	 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride, int srcOrder, int srcX, int srcY,
			int srcWidth, int srcHeight, int srcRedMask, int srcGreenMask, int srcBlueMask, int alphaMode,
			byte[] alphaData, int alphaStride, int alphaX, int alphaY, byte[] destData, int destDepth, int destStride,
			int destOrder, int destX, int destY, int destWidth, int destHeight, int destRedMask, int destGreenMask,
			int destBlueMask, boolean flipX, boolean flipY) {
		...
		int dp = dpr;
		int sp = spr;
		if (alphaMode == 0x10000 && stype == dtype && srcRedMask == destRedMask && srcGreenMask == destGreenMask
				&& srcBlueMask == destBlueMask && srcAlphaMask == destAlphaMask) {
			/*** Fast blit (straight copy) ***/
			switch (sbpp) {
			case 1:
				for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff)
						+ sfyi, dp = dpr += dpryi) {
		...
		if (alphaMode == 0x10000 && stype == TYPE_GENERIC_32_MSB && dtype == TYPE_GENERIC_32_MSB) {
			if (srcRedMask == 0xFF00 && srcGreenMask == 0xff0000 && srcBlueMask == 0xff000000
					&& destRedMask == 0xFF0000 && destGreenMask == 0xff00 && destBlueMask == 0xff) {
				for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff)
						+ sfyi, dp = dpr += dpryi) {
					for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
		...
		if (alphaMode == 0x10000 && stype == TYPE_GENERIC_24 && dtype == TYPE_GENERIC_32_MSB) {
			if (srcRedMask == 0xFF && srcGreenMask == 0xff00 && srcBlueMask == 0xff0000 && destRedMask == 0xFF0000
					&& destGreenMask == 0xff00 && destBlueMask == 0xff) {
				for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, sfy = (sfy & 0xffff)
						+ sfyi, dp = dpr += dpryi) {
					for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
						destData[dp] = 0;
		...
		for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, ap = apr += (sfy >>> 16)
				* alphaStride, sfy = (sfy & 0xffff) + sfyi, dp = dpr += dpryi) {
			for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
				/*** READ NEXT PIXEL ***/
				switch (stype) {
				case TYPE_GENERIC_8: {
				...
				case TYPE_GENERIC_32_MSB: {
					final int data = (((srcData[sp] & 0xff) << 8 | srcData[sp + 1] & 0xff) << 8 | srcData[sp + 2] & 0xff) << 8
							| srcData[sp + 3] & 0xff;
					sp += (sfx >>> 16) * 4;
					r = srcReds[(data & srcRedMask) >>> srcRedShift] & 0xff;
					g = srcGreens[(data & srcGreenMask) >>> srcGreenShift] & 0xff;
				...
				case TYPE_GENERIC_32_LSB: {
					final int data = (((srcData[sp + 3] & 0xff) << 8 | srcData[sp + 2] & 0xff) << 8 | srcData[sp + 1] & 0xff) << 8
							| srcData[sp] & 0xff;
					sp += (sfx >>> 16) * 4;
					r = srcReds[(data & srcRedMask) >>> srcRedShift] & 0xff;
					g = srcGreens[(data & srcGreenMask) >>> srcGreenShift] & 0xff;
				...
				if (alpha != 0x10000) {
					...
					case TYPE_GENERIC_16_LSB: {
						...
						break;
					case TYPE_GENERIC_24: {
						final int data = ((destData[dp] & 0xff) << 8 | destData[dp + 1] & 0xff) << 8 | destData[dp + 2]
								& 0xff;
						rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
						gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
						bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
						...
						break;
					case TYPE_GENERIC_32_MSB: {
						final int data = (((destData[dp] & 0xff) << 8 | destData[dp + 1] & 0xff) << 8 | destData[dp + 2] & 0xff) << 8
								| destData[dp + 3] & 0xff;
						rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
						gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
						bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
						...
						break;
					case TYPE_GENERIC_32_LSB: {
						final int data = (((destData[dp + 3] & 0xff) << 8 | destData[dp + 2] & 0xff) << 8 | destData[dp + 1] & 0xff) << 8
								| destData[dp] & 0xff;
						rq = destReds[(data & destRedMask) >>> destRedShift] & 0xff;
						gq = destGreens[(data & destGreenMask) >>> destGreenShift] & 0xff;
						bq = destBlues[(data & destBlueMask) >>> destBlueShift] & 0xff;
				...
				/*** WRITE NEXT PIXEL ***/
				final int data = r >>> destRedPreShift << destRedShift | g >>> destGreenPreShift << destGreenShift
						| b >>> destBluePreShift << destBlueShift | a >>> destAlphaPreShift << destAlphaShift;
				switch (dtype) {
				case TYPE_GENERIC_8: {
	 ...
	 * <p>
	 * Note: The source and destination red, green, and blue arrays may be null
	 * if no alpha blending or dither is to be performed.
	 * </p>
	 * 
	 * @param op
	 *            the blitter operation: a combination of BLIT_xxx flags (see
	 ...
	 *            the source byte array containing image data
	 * @param srcDepth
	 *            the source depth: one of 1, 2, 4, 8
	 * @param srcStride
	 *            the source number of bytes per line
	 * @param srcOrder
	 *            the source byte ordering: one of MSB_FIRST or LSB_FIRST;
	 ...
	 *            the height of the source blit region
	 * @param srcReds
	 *            the source palette red component intensities
	 * @param srcGreens
	 *            the source palette green component intensities
	 * @param srcBlues
	 *            the source palette blue component intensities
	 * @param alphaMode
	 *            the alpha blending or mask mode, may be an integer 0-255 for
	 *            global alpha; ignored if BLIT_ALPHA not specified in the
	 *            blitter operations (see ALPHA_MODE_xxx constants)
	 ...
	 *            the destination byte array containing image data
	 * @param destDepth
	 *            the destination depth: one of 1, 2, 4, 8
	 * @param destStride
	 *            the destination number of bytes per line
	 * @param destOrder
	 *            the destination byte ordering: one of MSB_FIRST or LSB_FIRST;
	 ...
	 *            the height of the destination blit region
	 * @param destReds
	 *            the destination palette red component intensities
	 * @param destGreens
	 *            the destination palette green component intensities
	 * @param destBlues
	 *            the destination palette blue component intensities
	 * @param flipX
	 *            if true the resulting image is flipped along the vertical axis
	 * @param flipY
	 *            if true the resulting image is flipped along the horizontal
	 ...
	 */
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride, int srcOrder, int srcX, int srcY,
			int srcWidth, int srcHeight, byte[] srcReds, byte[] srcGreens, byte[] srcBlues, int alphaMode,
			byte[] alphaData, int alphaStride, int alphaX, int alphaY, byte[] destData, int destDepth, int destStride,
			int destOrder, int destX, int destY, int destWidth, int destHeight, byte[] destReds, byte[] destGreens,
			byte[] destBlues, boolean flipX, boolean flipY) {
		if (destWidth <= 0 || destHeight <= 0 || alphaMode == ALPHA_TRANSPARENT) {
			return;
		}
		...
		case 0x10000:
			/***
			 * If the palettes and formats are equivalent use a one-to-one
			 * mapping
			 ***/
		...
		for (int dy = destHeight, sfy = sfyi; dy > 0; --dy, sp = spr += (sfy >>> 16) * srcStride, ap = apr += (sfy >>> 16)
			...
			for (int dx = destWidth, sfx = sfxi; dx > 0; --dx, dp += dprxi, sfx = (sfx & 0xffff) + sfxi) {
				...
				case ALPHA_MASK_RGB: {
					int i = 0;
					while (i < alphaData.length) {
						if (r == (alphaData[i] & 0xff) && g == (alphaData[i + 1] & 0xff)
	 ...
	 * <p>
	 * Note: The source and destination masks and palettes must always be fully
	 * specified.
	 * </p>
	 * 
	 * @param op
	...
	static void blit(int op, byte[] srcData, int srcDepth, int srcStride, int srcOrder, int srcX, int srcY,
		...
		if (srcX == 0 && srcY == 0 && destX == 0 && destY == 0 && destWidth == srcWidth && destHeight == srcHeight) {
			if (destDepth == 24 && srcDepth == 8 && (op & BLIT_ALPHA) == 0 && destRedMask == 0xFF0000
					&& destGreenMask == 0xFF00 && destBlueMask == 0xFF) {
			...
			if (destDepth == 32 && destOrder == MSB_FIRST && srcDepth == 8 && (op & BLIT_ALPHA) == 0
					&& destRedMask == 0xFF0000 && destGreenMask == 0xFF00 && destBlueMask == 0xFF) {
	 ...
	 * 
	 * @param gc
	 *            the GC to render the gradient onto
	 * @param device
	 *            the device the GC belongs to
	 * @param x
	 *            the top-left x coordinate of the region to be filled
	 * @param y
	 *            the top-left y coordinate of the region to be filled
	 * @param width
	 *            the width of the region to be filled
	 * @param height
	 *            the height of the region to be filled
	...
	static void fillGradientRectangle(GC gc, Device device, int x, int y, int width, int height, boolean vertical,
			RGB fromRGB, RGB toRGB, int redBits, int greenBits, int blueBits) {

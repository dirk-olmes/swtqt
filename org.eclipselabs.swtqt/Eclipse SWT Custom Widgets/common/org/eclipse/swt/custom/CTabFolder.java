...
public class CTabFolder extends Composite {
	...
	// highlight.
	// We are relying on the fact that only one tab state usually gets a
	// highlight, so only
	// a single cache is required. If that happens to not be true, cache simply
	// becomes less effective,
	// but we don't leak colours.
	Color[] selectionHighlightGradientColorsCache = null; // null is a legal
	// value, check on
	// access
	...
	public CTabFolder(Composite parent, int style) {
		...
		init(style);
		getQWidget().setObjectName("CTabFolder"); //$NON-NLS-1$
	}
	...
	void init(int style) {
		...
		int[] folderEvents = new int[] { SWT.Dispose, SWT.DragDetect, SWT.FocusIn, SWT.FocusOut, SWT.KeyDown,
				SWT.MouseDoubleClick, SWT.MouseDown, SWT.MouseEnter, SWT.MouseExit, SWT.MouseMove, SWT.MouseUp,
	...
	void antialias(int[] shape, RGB lineRGB, RGB innerRGB, RGB outerRGB, GC gc) {
		...
		// already does it. The simple style also does not require
		// anti-aliasing.
		if (simple) {
			return;
		}
	...
	public Rectangle computeTrim(int x, int y, int width, int height) {
		...
		if (minimized) {
			...
			int trimY = onBottom ? y - marginHeight - highlight_margin - borderTop : y - marginHeight
		...
		if (!minimized) {
			...
			if (highlight_margin > 0) {
				...
				if (onBottom) {
					...
					shape = new int[] { x1, y1, x1 + highlight_margin, y1, x1 + highlight_margin,
							y2 - highlight_margin, x2 - highlight_margin, y2 - highlight_margin, x2 - highlight_margin,
							y1, x2, y1, x2, y2, x1, y2 };
				}
				// If horizontal gradient, show gradient across the whole area
				if (selectedIndex != -1 && selectionGradientColors != null && selectionGradientColors.length > 1
						&& !selectionGradientVertical) {
					drawBackground(gc, shape, true);
				} else if (selectedIndex == -1 && gradientColors != null && gradientColors.length > 1
						&& !gradientVertical) {
	 ...
	 * then "Tab 3". If the CTabFolder is now compressed so that only two tabs
	...
	void onDragDetect(Event event) {
		//System.out.println("CTabFolder.onDragDetect: " + event);
		boolean consume = false;
		if (chevronRect.contains(event.x, event.y) || minRect.contains(event.x, event.y)
	...
	void redrawTabs() {
		Point size = getSize();
		if (onBottom) {
			redraw(0, size.y - borderBottom - tabHeight - highlight_header - 1, size.x, borderBottom + tabHeight
					+ highlight_header + 1, false);
		} else {
	 ...
	 * 				display.getSystemColor(SWT.COLOR_WHITE), display.getSystemColor(SWT.COLOR_WHITE) }, new int[] { 25, 50,
	 * 				100 });
	 * </pre>
	 * 
	 * @param colors
	...
	void setButtonBounds() {
		...
		if (showMax) {
			...
			maxRect.y = onBottom ? size.y - borderBottom - tabHeight + (tabHeight - BUTTON_SIZE) / 2 : borderTop
		...
		if (showMin) {
			...
			minRect.y = onBottom ? size.y - borderBottom - tabHeight + (tabHeight - BUTTON_SIZE) / 2 : borderTop
					+ (tabHeight - BUTTON_SIZE) / 2;
		...
		if (oldX != topRightRect.x || oldWidth != topRightRect.width || oldY != topRightRect.y
				|| oldHeight != topRightRect.height) {
		...
		if (oldX != chevronRect.x || oldWidth != chevronRect.width || oldY != chevronRect.y
				|| oldHeight != chevronRect.height) {
	 ...
	 * 
	 * @param index
	 *            the index of the item with which the mark is associated or
	 *            null
	 * 
	 * @param after
	 *            true if the mark should be displayed after the specified item
	...
	void setSelectionHighlightGradientColor(Color start) {
		...
		// For early returns, don't realloc the cache, we may get a cache hit
		// next time we're given the highlight
		...
		if (!isSelectionHighlightColorsCacheHit(start)) {
			createSelectionHighlightGradientColors(start); // if no cache hit
			// then compute
			// new ones
		}
	}
	...
	boolean isSelectionHighlightColorsCacheHit(Color start) {
		...
		// this case should never happen but check to be safe before accessing
		// array indexes
		...
		// Compare existing highlight end to what it would be
		// (selectionBackground)
	 ...
	 * height changes. Could remain null if there'd be no gradient (start=end or
	 * low colour display)
	 */
	void createSelectionHighlightGradientColors(Color start) {
		...
		// alloc colours for entire height to ensure it matches wherever we stop
		// drawing
	...
	/*
	 * Return the gradient start color for selected tabs, which is the start of
	 * the tab fade (end is selectionBackground).
	 */
	...
	boolean updateTabHeight(boolean force) {
		...
		if (fixedTabHeight != SWT.DEFAULT) {
			tabHeight = fixedTabHeight == 0 ? 0 : fixedTabHeight + 1; // +1 for
			// line
			// drawn
			// across
			// top
		...
		if (onBottom) {
			...
			// this could be static but since values depend on curve, better to
			// keep in one place
			topCurveHighlightStart = new int[] { 0, 2, 1, 2, 2, 2, 3, 3, 4, 3, 5, 3, 6, 4, 7, 4, 8, 5, 9, 6, 10, 6 };

			// also, by adding in 'd' here we save some math cost when drawing
			// the curve

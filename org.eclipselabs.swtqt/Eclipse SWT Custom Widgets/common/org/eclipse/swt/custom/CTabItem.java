...
public class CTabItem extends Item {
	 ...
	 * <code>CTabFolder</code>) and a style value describing its behavior and
	 ...
	 * 
	 * @param parent
	 *            a CTabFolder which will be the parent of the new instance
	 *            (cannot be null)
	 * @param style
	...
	/**
	 * Constructs a new instance of this class given its parent (which must be a
	 * <code>CTabFolder</code>), a style value describing its behavior and
	...
	void drawClose(GC gc) {
		...
		case CTabFolder.NORMAL: {
			int[] shape = new int[] { x, y, x + 2, y, x + 4, y + 2, x + 5, y + 2, x + 7, y, x + 9, y, x + 9, y + 2,
					x + 7, y + 4, x + 7, y + 5, x + 9, y + 7, x + 9, y + 9, x + 7, y + 9, x + 5, y + 7, x + 4, y + 7,
					x + 2, y + 9, x, y + 9, x, y + 7, x + 2, y + 5, x + 2, y + 4, x, y + 2 };
		...
		case CTabFolder.SELECTED: {
			int[] shape = new int[] { x + 1, y + 1, x + 3, y + 1, x + 5, y + 3, x + 6, y + 3, x + 8, y + 1, x + 10,
					y + 1, x + 10, y + 3, x + 8, y + 5, x + 8, y + 6, x + 10, y + 8, x + 10, y + 10, x + 8, y + 10,
					x + 6, y + 8, x + 5, y + 8, x + 3, y + 10, x + 1, y + 10, x + 1, y + 8, x + 3, y + 6, x + 3, y + 5,
					x + 1, y + 3 };
			gc.setBackground(parent.getFillColor());
			gc.fillPolygon(shape);
			gc.setForeground(closeBorder);
	...
	void drawSelected(GC gc) {
		...
		int yy = parent.onBottom ? size.y - parent.borderBottom - parent.tabHeight - parent.highlight_header
				: parent.borderTop + parent.tabHeight + 1;
		...
		if (parent.single) {
			...
			if (this.parent.onBottom) {
				...
				for (int i = 0; i < right.length / 2; i++) {
					shape[index++] = parent.simple ? rightEdge - 1 + right[2 * i] : rightEdge - parent.curveIndent
							+ right[2 * i];
			...
			if (tabInPaint) {
				...
				if (parent.selectionGradientColors != null && !parent.selectionGradientVertical) {
					...
					parent.drawBackground(gc, shape, xx, yy, ww, hh, defaultBackground, image, colors, percents,
							vertical);
				}
			}
			...
			if (parent.selectionBgImage != null || parent.selectionGradientColors != null
	 ...
	 * Only for curved tabs, on top. Do not draw if insufficient colors.
	 */
	 ...
	 * area of the CTabFolder. Returns false otherwise.
	 * 
	...
	/**
	 * Sets the image that is displayed if the tab item is disabled. Null will
	 * clear the image.
	 * 
	 * @param image
	 ...
	 * should be shown. If the parent (CTabFolder) was created with SWT.CLOSE

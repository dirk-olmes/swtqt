...
/**
 * A ScrolledComposite provides scrollbars and will scroll its content when the
 * user uses the scrollbars.
 * 
 * 
 * <p>
 ...
 * ScrolledComposite will show scrollbars when the contained control can not be
 * fully seen.
 * 
 * 2) The second way imitates the way a browser would work. Set the minimum size
 * of the control and the ScrolledComposite will show scroll bars if the visible
 * area is less than the minimum size of the control and it will expand the size
 * of the control if the visible area is greater than the minimum size. This
 * requires invoking both setMinWidth(), setMinHeight() and
 * setExpandHorizontal(), setExpandVertical().
 * 
 * <code><pre>
 ...
 * <dl>
 * <dt><b>Styles:</b>
 * <dd>H_SCROLL, V_SCROLL
 * </dl>
 * 
 * @see <a
 *      href="http://www.eclipse.org/swt/snippets/#scrolledcomposite">ScrolledComposite
 *      snippets</a>
...
public class ScrolledComposite extends Composite {
	 ...
	 * always shown even if they are not required. False if the scrollbars are
	 * only visible when some part of the composite needs to be scrolled to be
	 * seen. The H_SCROLL and V_SCROLL style bits are also required to enable
	 * scrollbars in the horizontal and vertical directions.
	 * 
	...
	/**
	 * Set the Always Show Scrollbars flag. True if the scrollbars are always
	 * shown even if they are not required. False if the scrollbars are only
	 * visible when some part of the composite needs to be scrolled to be seen.
	 * The H_SCROLL and V_SCROLL style bits are also required to enable
	 * scrollbars in the horizontal and vertical directions.
	 * 
	 * @param show
	 *            true to show the scrollbars even when not required, false to
	 *            show scrollbars only when required
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been

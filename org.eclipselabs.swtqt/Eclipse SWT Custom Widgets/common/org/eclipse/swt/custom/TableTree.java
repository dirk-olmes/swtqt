...
public class TableTree extends Composite {
	...
	/*
	 * TableTreeItems are not treated as children but rather as items. When the
	 * TableTree is disposed, all children are disposed because TableTree
	 * inherits this behaviour from Composite. The items must be disposed
	 * separately. Because TableTree is not part of the org.eclipse.swt.widgets
	 * package, the method releaseWidget can not be overridden (this is how
	 * items are disposed of in Table and Tree). Instead, the items are disposed
	 * of in response to the dispose event on the TableTree. The "inDispose"
	 * flag is used to distinguish between disposing one TableTreeItem (e.g.
	 * when removing an entry from the TableTree) and disposing the entire
	 * TableTree.
	 */
	 ...
	 *                thread <li>ERROR_WIDGET_DISPOSED when the widget has been
	 *                disposed
	 *                </ul>
	 */
	 ...
	 * <p>
	 * This operation will fail if the selected items cannot be queried from the
	 * OS.
	 * 
	 ...
	 * <p>
	 * This operation will fail if the number of selected items cannot be
	 * queried from the OS.
	 * 
	 ...
	 * 
	 * <p>
	 * The widget is searched starting at 0 until an item is found that is equal
	 * to the search item. If no item is found, -1 is returned. Indexing is zero
	 * based. This index is relative to the parent only.
	 * 
	 * @param item
	 *            the search item
	...
	void onDispose(Event e) {
		 ...
		 * the items array and dispose of the underlying table items. Since the
		 * whole table tree is being disposed, this is not necessary. For speed
		 * the inDispose flag is used to skip over this part of the item
		 * dispose.
		 */
		inDispose = true;
		for (int i = 0; i < items.length; i++) {
	 ...
	 *                </li> <li>ERROR_INVALID_ARGUMENT - if one of the item has
	 *                been disposed</li>
	 *                </ul>
	 * @exception SWTException
	 *                <ul>
	 ...
	 * Shows the selection.
	 * <p>
	 * If there is no selection or the selection is already visible, this method
	 * does nothing. If the selection is scrolled out of view, the top index of
	 * the widget is changed such that selection becomes visible.
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_THREAD_INVALID_ACCESS when called from the wrong

...
public class CLabel extends Canvas {
	 ...
	 * 
	 * <pre>
	 * clabel.setBackground(new Color[] { display.getSystemColor(SWT.COLOR_DARK_BLUE), display.getSystemColor(SWT.COLOR_BLUE),
	 * 		display.getSystemColor(SWT.COLOR_WHITE), display.getSystemColor(SWT.COLOR_WHITE) }, new int[] { 25, 50, 100 });
	 * </pre>
	 * 
	 * @param colors
		...
		if (backgroundImage == null) {
			if (gradientColors != null && colors != null && gradientColors.length == colors.length) {
				...
				for (int i = 0; i < gradientColors.length; i++) {
					same = gradientColors[i] == colors[i] || gradientColors[i] == null && colors[i] == background

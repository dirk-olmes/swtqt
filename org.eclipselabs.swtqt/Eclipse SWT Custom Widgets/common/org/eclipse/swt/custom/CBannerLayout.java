...
class CBannerLayout extends Layout {
	...
	protected Point computeSize(Composite composite, int wHint, int hHint, boolean flushCache) {
		...
		if (left != null) {
			if (right == null) {
				...
				height += Math.max(leftSize.y, banner.rightMinHeight == SWT.DEFAULT ? rightSize.y
	...
	protected void layout(Composite composite, boolean flushCache) {
		...
		if (right != null) {
			if (left != null) {
				rightSize.y = Math.max(leftSize.y, banner.rightMinHeight == SWT.DEFAULT ? rightSize.y
						: banner.rightMinHeight);
			}
			rightRect = new Rectangle(x, y, rightSize.x, rightSize.y);
		}
		if (banner.curveStart < oldStart) {
			banner.redraw(banner.curveStart - CBanner.CURVE_TAIL, 0, oldStart + banner.curve_width - banner.curveStart
					+ CBanner.CURVE_TAIL + 5, size.y, false);
		}
		if (banner.curveStart > oldStart) {
			banner.redraw(oldStart - CBanner.CURVE_TAIL, 0, banner.curveStart + banner.curve_width - oldStart
					+ CBanner.CURVE_TAIL + 5, size.y, false);
		}
		/*
		 * The paint events must be flushed in order to make the curve draw
		 * smoothly while the user drags the divider. On Windows, it is
		 * necessary to flush the paints before the children are resized because
		 * otherwise the children (particularly toolbars) will flash.
		 */
		banner.update();

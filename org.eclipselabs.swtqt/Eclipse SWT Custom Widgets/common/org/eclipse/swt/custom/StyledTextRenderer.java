...
/**
 * A StyledTextRenderer renders the content of a StyledText widget. This class
 * can be used to render to the display or to a printer.
 */
class StyledTextRenderer {
	...
	int addMerge(int[] mergeRanges, StyleRange[] mergeStyles, int mergeCount, int modifyStart, int modifyEnd) {
		...
		if (rangeCount + grow >= ranges.length) {
			...
			if (rangeCount > modifyEnd) {
				...
				System.arraycopy(styles, modifyEnd >> 1, tmpStyles, modifyStart + mergeCount >> 1, styleCount
						- (modifyEnd >> 1));
			}
			ranges = tmpRanges;
			styles = tmpStyles;
			...
			if (rangeCount > modifyEnd) {
				...
				System.arraycopy(styles, modifyEnd >> 1, styles, modifyStart + mergeCount >> 1, styleCount
						- (modifyEnd >> 1));
			}
		}
		if (MERGE_STYLES) {
			...
			for (int i = 0; i < mergeCount; i += 2) {
				if (j > 0 && ranges[j - 2] + ranges[j - 1] == mergeRanges[i]
	...
	int addMerge(StyleRange[] mergeStyles, int mergeCount, int modifyStart, int modifyEnd) {
		...
		if (MERGE_STYLES) {
			...
			for (int i = 0; i < mergeCount; i++) {
				...
				if (j > 0 && (style = styles[j - 1]).start + style.length == newStyle.start
	...
	int drawLine(int lineIndex, int paintX, int paintY, GC gc, Color widgetBackground, Color widgetForeground) {
		...
		if (bulletIndex != -1 && bullet != null) {
			...
			if (bullet.type == ST.BULLET_CUSTOM) {
				...
				styledText.paintObject(gc, paintX, paintY, lineAscent, metrics.getDescent(), bullet.style, bullet,
						bulletIndex);
			} else {
	...
	boolean hasLink(int offset) {
		...
		if (event != null) {
			...
			if (styles != null) {
				...
				if (ranges != null) {
					for (int i = 0; i < ranges.length; i += 2) {
						if (ranges[i] <= offset && offset < ranges[i] + ranges[i + 1] && styles[i >> 1].underline
								&& styles[i >> 1].underlineStyle == SWT.UNDERLINE_LINK) {
							return true;
						}
					}
				} else {
			...
			if (ranges != null) {
				...
				if (rangeStart <= offset && offset < rangeStart + rangeLength && rangeStyle.underline
						&& rangeStyle.underlineStyle == SWT.UNDERLINE_LINK) {
					return true;
				}
			}
		}
	...
	void setStyleRanges(int[] newRanges, StyleRange[] newStyles) {
		...
		if (ranges != null) {
			...
			for (int i = 0; i < newRanges.length; i += 2) {
				...
				mergeCount += 2;
				if (modifyEnd < rangeCount && ranges[modifyEnd] < newEnd
	...
	void updateRanges(int start, int replaceCharCount, int newCharCount) {
		...
		if (ranges != null) {
			...
			if (modifyStart == modifyEnd && ranges[modifyStart] < start
				...
				if (newCharCount == 0) {
					...
					System.arraycopy(styles, modifyStart + 2 >> 1, styles, modifyStart + 4 >> 1, styleCount
							- (modifyStart + 2 >> 1));
				...
				if (modifyEnd < rangeCount && ranges[modifyEnd] < end
						&& end < ranges[modifyEnd] + ranges[modifyEnd + 1]) {
			...
			int offset = newCharCount - replaceCharCount;
			if (modifyStart == modifyEnd && styles[modifyStart].start < start
				...
				if (modifyEnd < styleCount && styles[modifyEnd].start < end
						&& end < styles[modifyEnd].start + styles[modifyEnd].length) {

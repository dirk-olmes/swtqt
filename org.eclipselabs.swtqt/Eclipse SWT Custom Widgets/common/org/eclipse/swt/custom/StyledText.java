 ...
 * </ul>
 * <p>
 * In addition to text style attributes, the background color of a line may be
 * specified.
 * </p>
 * <p>
 ...
 * You may use the API that is defined for StyledText or you may define your own
 * LineBackgroundListener. If you define your own listener, you will be
 * responsible for maintaining the line background color information for the
 * widget. IMPORTANT: You may not define your own listener and use the
 * StyledText API. The following StyledText API is not supported if you have
 * defined a LineBackgroundListener:
 * <ul>
 ...
 * </p>
 * <p>
 * The content implementation for this widget may also be user-defined. To do
 * so, you must implement the StyledTextContent interface and use the StyledText
 * API setContent(StyledTextContent) to initialize the widget.
 * </p>
 * <p>
 * <dl>
 * <dt><b>Styles:</b>
 * <dd>FULL_SELECTION, MULTI, READ_ONLY, SINGLE, WRAP
 * <dt><b>Events:</b>
 * <dd>ExtendedModify, LineGetBackground, LineGetSegments, LineGetStyle, Modify,
 * Selection, Verify, VerifyKey
 * </dl>
 * </p>
 * <p>
 ...
 * </p>
 * 
 * @see <a href="http://www.eclipse.org/swt/snippets/#styledtext">StyledText
 *      snippets</a>
 * @see <a href="http://www.eclipse.org/swt/examples.php">SWT Examples:
 *      CustomControlExample, TextEditor</a>
...
public class StyledText extends Canvas {
	...
	/**
	 * The Printing class implements printing of a range of text. An instance of
	 * <code>Printing</code> is returned in the StyledText#print(Printer) API.
	 * The run() method may be invoked from any thread.
	 */
	static class Printing implements Runnable {
		...
		/**
		 * Creates an instance of <code>Printing</code>. Copies the widget
		 * content and rendering data that needs to be requested from listeners.
		 * </p>
		 * 
		 * @param parent
		 *            StyledText widget to print.
		 * @param printer
		 *            printer device to print on.
		 * @param printOptions
		 *            print options
		 */
		...
		/**
		 * Caches all line data that needs to be requested from a listener. </p>
		 * 
		 * @param printerContent
		 *            <code>StyledTextContent</code> to request line data for.
		 */
		void cacheLineData(StyledText styledText) {
			...
			if (styledText.isListening(LineGetBackground) || styledText.isBidi()
					&& styledText.isListening(LineGetSegments) || styledText.isListening(LineGetStyle)) {
		...
		/**
		 * Copies the text of the specified <code>StyledTextContent</code>. </p>
		 * 
		 * @param original
		 *            the <code>StyledTextContent</code> to copy.
		 */
		StyledTextContent copyContent(StyledTextContent original) {
			...
			for (int i = 0; i < original.getLineCount(); i++) {
				...
				printerContent.replaceTextRange(insertOffset, 0, original.getTextRange(insertOffset, insertEndOffset
						- insertOffset));
		 ...
		 * 
		 * @param page
		 *            page number to print, if specified in the
		 *            StyledTextPrintOptions header or footer.
		 * @param header
		 *            true = print the header, false = print the footer
		 */
		...
		/**
		 * Print one segment of a header or footer decoration. Headers and
		 * footers have three different segments. One each for left aligned,
		 * centered, and right aligned text.
		 * 
		 * @param segment
		 *            decoration segment to print
		 * @param alignment
		 *            alignment of the segment. 0=left, 1=center, 2=right
		 * @param page
		 *            page number to print, if specified in the decoration
		 *            segment.
		 * @param header
		 *            true = print the header, false = print the footer
		 */
	...
	/**
	 * The <code>RTFWriter</code> class is used to write widget content as rich
	 * text. The implementation complies with the RTF specification version 1.5.
	 * <p>
	 * toString() is guaranteed to return a valid RTF string only after close()
	 * has been called.
	 * </p>
	 * <p>
	 * Whole and partial lines and line breaks can be written. Lines will be
	 * formatted using the styles queried from the LineStyleListener, if set, or
	 * those set directly in the widget. All styles are applied to the RTF
	 * stream like they are rendered by the widget. In addition, the widget font
	 * name and size is used for the whole text.
	 * </p>
	 */
	class RTFWriter extends TextWriter {
		 ...
		 * in the document. <code>start</code> and <code>length</code>can be set
		 ...
		 * <b>NOTE:</b> <code>toString()</code> does not return a valid RTF
		 * string until <code>close()</code> has been called.
		 */
		@Override
		public void close() {
			if (!isClosed()) {
		 ...
		 *            the color
		 * @param defaultIndex
		 *            return value if color is null
		 * @return the index of the specified color in the RTF color table or
		 *         "defaultIndex" if "color" is null.
		 */
		...
		/**
		 * Determines if Unicode RTF should be written. Don't write Unicode RTF
		 * on Windows 95/98/ME or NT.
		 */
		void setUnicode() {
			...
			WriteUnicode = !osName.startsWith(Win95) && !osName.startsWith(Win98) && !osName.startsWith(WinME)
					&& (!osName.startsWith(WinNT) || majorVersion > 4);
		}

		/**
		 * Appends the specified segment of "string" to the RTF data. Copy from
		 * <code>start</code> up to, but excluding, <code>end</code>.
		 * 
		 * @param string
		 *            string to copy a segment from. Must not contain line
		 *            breaks. Line breaks should be written using
		 *            writeLineDelimiter()
		 * @param start
		 *            start offset of segment. 0 based.
		 * @param end
		 *            end offset of segment
		 */
		...
		/**
		 * Appends the specified line text to the RTF data. Lines will be
		 * formatted using the styles queried from the LineStyleListener, if
		 * set, or those set directly in the widget.
		 * 
		 * @param line
		 ...
		 * 
		 * @param lineDelimiter
		 *            line delimiter to write as RTF.
		 * @exception SWTException
		 *                <ul>
		 *                <li>ERROR_IO when the writer is closed.</li>
		 *                </ul>
		 ...
		 * <p>
		 * Use the colors and font styles specified in "styles" and
		 * "lineBackground". Formatting is written to reflect the text rendering
		 * by the text widget. Style background colors take precedence over the
		 * line background color. Background colors are written using the
		 * \highlight tag (vs. the \cb tag).
		 * </p>
		 * 
		 * @param line
		 *            line text to write as RTF. Must not contain line breaks
		 ...
		 *            ignored.
		 * @param styles
		 *            styles to use for formatting. Must not be null.
		 * @param lineBackground
		 *            line background color to use for formatting. May be null.
		 */
	 ...
	 * <p>
	 * A BidiSegmentEvent is sent whenever a line of text is measured or
	 * rendered. The user can specify text ranges in the line that should be
	 * treated as if they had a different direction than the surrounding text.
	 * This may be used when adjacent segments of right-to-left text should not
	 * be reordered relative to each other. E.g., Multiple Java string literals
	 * in a right-to-left language should generally remain in logical order to
	 * each other, that is, the way they are stored.
	 * </p>
	 * 
	 * @param listener
	 *            the listener
	...
	/**
	 * Adds a paint object listener. A paint object event is sent by the widget
	...
	/**
	 * Adds a verify listener. A Verify event is sent by the widget when the
	 * widget text is about to change. The listener can set the event text and
	 * the doit field to change the text that is set in the widget or to force
	 * the widget to ignore the text change.
	 * 
	...
	/**
	 * Calculates the top index based on the current vertical scroll offset. The
	 * top index is the index of the topmost fully visible line or the topmost
	 * partially visible line if no line is fully visible. The top index starts
	 * at 0.
	 */
		...
		if (isFixedLineHeight()) {
			...
			if (delta >= 0) {
				...
				if (lineIndex < lineCount
						&& -delta + renderer.getLineHeight(lineIndex) <= clientAreaHeight - topMargin - bottomMargin) {
					topIndex = lineIndex;
					topIndexY = -delta;
				} else {
	 ...
	 * 
	 * @param sendEvent
	 *            a Selection event is sent when set to true and when the
	 ...
	 * <p>
	 * The text will be put on the clipboard in plain text format and RTF
	 * format. The <code>DND.CLIPBOARD</code> clipboard is used for data that is
	 * transferred by keyboard accelerator (such as Ctrl+C/Ctrl+V) or by menu
	 * action.
	 * </p>
	 * 
	 * @exception SWTException
	...
	/**
	 * Copies the selected text to the specified clipboard. The text will be put
	 * in the clipboard in plain text format and RTF format.
	 * <p>
	 * The clipboardType is one of the clipboard constants defined in class
	 * <code>DND</code>. The <code>DND.CLIPBOARD</code> clipboard is used for
	 * data that is transferred by keyboard accelerator (such as Ctrl+C/Ctrl+V)
	 * or by menu action. The <code>DND.SELECTION_CLIPBOARD</code> clipboard is
	 * used for data that is transferred by selecting text and pasting with the
	 * middle mouse button.
	 * </p>
	 * 
	 * @param clipboardType
	 ...
	 * 
	 * @param text
	 *            the text that may have line delimiters that don't match the
	 *            model line delimiter. Possible line delimiters are CR ('\r'),
	 *            LF ('\n'), CR/LF ("\r\n")
	 * @return the converted text that only uses the line delimiter specified by
	 *         the model. Returns only the first line if the widget has the
	 *         SWT.SINGLE style.
	 */
	...
	/**
	 * Create the bitmaps to use for the caret in bidi mode. This method only
	 * needs to be called upon widget creation and when the font changes (the
	 * caret bitmap height needs to match font height).
	 */
	 ...
	 * 
	 * @param direction
	 *            SWT.UP, SWT.DOWN, SWT.COLUMN_NEXT, SWT.COLUMN_PREVIOUS
	 */
	...
	/**
	 * Deletes the previous character. Delete the selected text if any. Move the
	 * caret in front of the deleted text.
	 */
	void doBackspace() {
	 ...
	 * Otherwise, if no selection exists move the cursor according to the cursor
	 * selection rules.
	 * 
	...
	/**
	 * Moves the caret one line up and to the same character offset relative to
	 * the beginning of the line. Move the caret to the end of the new line if
	 * the new line is shorter than the character offset.
	 */
	 ...
	 * 
	 * @param x
	 *            x location of the new caret position
	 * @param y
	 *            y location of the new caret position
	 * @param select
	 *            the location change is a selection operation. include the line
	 *            delimiter in the selection
	 */
	...
	void doMouseSelection() {
		if (caretOffset <= selection.x || caretOffset > selection.x && caretOffset < selection.y
	...
	/**
	 * Returns the offset of the word at the specified offset. If the current
	 * selection extends from high index to low index (i.e., right to left, or
	 * caret is at left border of selection on non-bidi platforms) the start
	 * offset of the word preceding the selection is returned. If the current
	 * selection extends from low index to high index the end offset of the word
	 * following the selection is returned.
	 * 
	 * @param x
	 *            mouse x location
	 * @param newCaretOffset
	 *            caret offset of the mouse cursor location
	 * @param line
	 *            line index of the mouse cursor location
	 */
	...
	/**
	 * Scrolls one page down so that the last line (truncated or whole) of the
	 * current page becomes the fully visible top line.
	 * <p>
	 * The caret is scrolled the same number of lines so that its location
	 * relative to the top line remains the same. The exception is the end of
	 * the text where a full page scroll is not possible. In this case the caret
	 * is moved after the last character.
	 * </p>
	 * 
	 * @param select
	...
	/**
	 * Scrolls one page up so that the first line (truncated or whole) of the
	 * current page becomes the fully visible last line. The caret is scrolled
	 * the same number of lines so that its location relative to the top line
	 * remains the same. The exception is the beginning of the text where a full
	 * page scroll is not possible. In this case the caret is moved in front of
	 * the first character.
	 */
	...
	/**
	 * Moves the caret to the next character or to the beginning of the next
	 * line if the cursor is at the end of a line.
	 */
	 ...
	 * the end of the text if the caret already is on the last line. Adjusts the
	 * selection according to the caret change. This can either add to or
	 * subtract from the old selection, depending on the previous selection
	 * direction.
	 */
	...
	/**
	 * Moves the caret one line up and to the same character offset relative to
	 * the beginning of the line. Moves the caret to the end of the new line if
	 * the new line is shorter than the character offset. Moves the caret to the
	 * beginning of the document if it is already on the first line. Adjusts the
	 * selection according to the caret change. This can either add to or
	 * subtract from the old selection, depending on the previous selection
	 * direction.
	 */
	 ...
	 * to or subtract from the old selection, depending on the previous
	 * selection direction. </p>
	 */
	...
	/**
	 * Scrolls one page up so that the first line (truncated or whole) of the
	 * current page becomes the fully visible last line.
	 * <p>
	 * The caret is scrolled the same number of lines so that its location
	...
	/**
	 * Moves the caret one character to the left. Do not go to the previous
	 * line. When in a bidi locale and at a R2L character the caret is moved to
	 * the beginning of the R2L segment (visually right) and then one character
	 * to the left (visually left because it's now in a L2R segment).
	 */
	 ...
	 * the caret to the end of the selection and remove the selection.
	 */
	void doWordNext() {
		if (selection.y - selection.x > 0) {
	...
	/**
	 * Moves the caret to the start of the previous word. If a selection exists,
	 * move the caret to the start of the selection and remove the selection.
	 */
	void doWordPrevious() {
		if (selection.y - selection.x > 0) {
	 ...
	 * 
	 * Note: this API should not be used if a StyleRange attribute causes lines
	 * to have different heights (i.e. different fonts, rise, etc).
	 * 
	...
	/**
	 * Gets the BIDI coloring mode. When true the BIDI text display algorithm is
	 * applied to segments of text that are the same color.
	 * 
	...
	/**
	 * Returns the block selection bounds. The bounds is relative to the upper
	...
	public Rectangle getBlockSelectionBounds() {
		...
		if (blockSelection && blockXLocation != -1) {
			...
			rect = new Rectangle(startPoint.x, startPoint.y, endPoint.x - startPoint.x, endPoint.y + height
					- startPoint.y);
			if (selection.x == selection.y) {
	 ...
	 *         text storage or the default content implementation if no user
	 *         defined content implementation has been set.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * 
	 * @return true if double clicking a word selects the word, false if double
	 *         clicks have the same effect as regular mouse clicks
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 *         measured in character increments starting at 0, if > 0 the
	 *         content is scrolled
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	...
	/**
	 * Returns the action assigned to the key. Returns SWT.NULL if there is no
	 * action associated with the key.
	 * 
	 * @param key
	 ...
	 * @return one of the predefined actions defined in ST.java or SWT.NULL if
	 *         there is no action associated with the key.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	...
	/**
	 * Returns the line at the given line index without delimiters. Index 0 is
	 * the first line of the content. When there are not any lines, getLine(0)
	 * is a valid call that answers an empty string.
	 * <p>
	 * 
	 * @param lineIndex
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_RANGE when the line index is outside the
	 *                valid range (< 0 or >= getLineCount())</li>
	 *                </ul>
	 * @since 3.4
	 */
	...
	/**
	 * Returns the line at the specified offset in the text where 0 &lt; offset
	 * &lt; getCharCount() so that getLineAtOffset(getCharCount()) returns the
	 * line of the insert location.
	 * 
	 * @param offset
	 *            offset relative to the start of the content. 0 <= offset <=
	 *            getCharCount()
	...
	/**
	 * Returns the background color of the line at the given index. Returns null
	 * if a LineBackgroundListener has been set or if no background color has
	 * been specified for the line. Should not be called if a
	 * LineBackgroundListener has been set since the listener maintains the line
	 * background colors.
	 * 
	 * @param index
	 *            the index of the line
	...
	/**
	 * Returns the line background data for the given line or null if there is
	 * none.
	 * 
	 * @param lineOffset
	 *            offset of the line start relative to the start of the content.
	 * @param line
	 ...
	 * @return line style data for the given line. Styles may start before line
	 *         start and end after line end
	 */
	 ...
	 * 
	 * @param lineIndex
	 *            the line index, the max value is lineCount. If lineIndex ==
	 *            lineCount it returns the bottom pixel of the last line. It
	 *            means this function can be used to retrieve the bottom pixel
	 *            of any line.
	 * 
	...
	/**
	 * Returns the line index for a y, relative to the client area. The line
	 * index returned is always in the range 0..lineCount - 1.
	 * 
	 * @param y
	 ...
	 *            content. 0 <= lineIndex < getLineCount(), except lineIndex may
	 *            always be 0
	 * @return offset offset of the first character of the line, relative to the
	 *         beginning of the document. The first character of the document is
	 *         at offset 0. When there are not any lines, getOffsetAtLine(0) is
	 *         a valid call that answers 0.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	...
	/**
	 * Returns the offset of the character at the given location relative to the
	 * first character in the document.
	 * <p>
	 * The return value reflects the character offset that the caret will be
	 * placed at if a mouse click occurred at the specified location. If the x
	 * coordinate of the location is beyond the center of a character the
	 * returned offset will be behind the character.
	 * </p>
	 * 
	 * @param point
	 *            the origin of character bounding box relative to the origin of
	 *            the widget client area.
	 * @return offset of the character at the given location relative to the
	 *         first character in the document.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 *                <li>ERROR_INVALID_ARGUMENT when there is no character at
	 ...
	 *         delimiter to separate lines as written by the specified
	 *         TextWriter.
	 */
	 ...
	 * Returns an empty array if a LineStyleListener has been set. Should not be
	 * called if a LineStyleListener has been set since the listener maintains
	 * the styles.
	 * <p>
	 * The ranges array contains start and length pairs. Each pair refers to the
	 * corresponding style in the styles array. For example, the pair that
	 * starts at ranges[n] with length ranges[n+1] uses the style at styles[n/2]
	 * returned by <code>getStyleRanges(int, int, boolean)</code>.
	 * </p>
	 * 
	...
	/**
	 * Returns the ranges of text that have an associated StyleRange. Returns an
	 * empty array if a LineStyleListener has been set. Should not be called if
	 * a LineStyleListener has been set since the listener maintains the styles.
	 * <p>
	 * The ranges array contains start and length pairs. Each pair refers to the
	 * corresponding style in the styles array. For example, the pair that
	 * starts at ranges[n] with length ranges[n+1] uses the style at styles[n/2]
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_RANGE if start or length are outside the
	 *                widget content</li>
	 *                </ul>
	 * 
	 * @since 3.2
	 ...
	 * Returns the selection.
	 * 
	 * @return start and length of the selection, x is the offset of the first
	 *         selected character, relative to the first character of the widget
	 *         content. y is the length of the selection. The selection values
	 *         returned are visual (i.e., length will always always be
	 *         positive). To determine if a selection is right-to-left (RtoL)
	 *         vs. left-to-right (LtoR), compare the caretOffset to the start
	 *         and end of the selection (e.g., caretOffset == start of selection
	 *         implies that the selection is RtoL).
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * <p>
	 * The ranges array contains start and length pairs. When the receiver is
	 * not in block selection mode the return arrays contains the start and
	 * length of the regular selection.
	 * 
	...
	/**
	 * Returns the text segments that should be treated as if they had a
	 * different direction than the surrounding text.
	 * 
	 * @param lineOffset
	 *            offset of the first character in the line. 0 based from the
	 *            beginning of the document.
	 * @param line
	 *            text of the line to specify bidi segments for
	 * @return text segments that should be treated as if they had a different
	 *         direction than the surrounding text. Only the start index of a
	 *         segment is specified, relative to the start of the line. Always
	 *         starts with 0 and ends with the line length.
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT - if the segment indices
	 *                returned by the listener do not start with 0, are not in
	 *                ascending order, exceed the line length or have duplicates
	 *                </li>
	 *                </ul>
	 */
	...
	/**
	 * @see #getBidiSegments Supports deprecated setBidiColoring API. Remove
	 *      when API is removed.
	 */
	int[] getBidiSegmentsCompatibility(String line, int lineOffset) {
		...
		for (int i = k; i < styles.length; i++) {
			...
			if (i > 0
					&& count > 1
					&& (styleLineStart >= offsets[count - 2] && styleLineStart <= offsets[count - 1] || styleLineEnd >= offsets[count - 2]
							&& styleLineEnd <= offsets[count - 1]) && style.similarTo(styles[i - 1])) {
	 ...
	 * for the offset. Should not be called if a LineStyleListener has been set
	 * since the listener maintains the styles.
	 * </p>
	 * 
	 * @param offset
	 *            the offset to return the style for. 0 <= offset <
	 *            getCharCount() must be true.
	 * @return a StyleRange with start == offset and length == 1, indicating the
	 *         style at the given offset. null if a LineStyleListener has been
	 *         set or if a style is not set for the given offset.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * <p>
	 * </p>
	 * Note: Because a StyleRange includes the start and length, the same
	 * instance cannot occur multiple times in the array of styles. If the same
	 * style attributes, such as font and color, occur in multiple StyleRanges,
	 * <code>getStyleRanges(boolean)</code> can be used to get the styles
	 * without the ranges. </p>
	 * 
	 ...
	 * fields of each StyleRange will be valid, however the StyleRange objects
	 * may need to be cloned. When <code>includeRanges</code> is false,
	 * <code>getRanges(int, int)</code> can be used to get the associated
	 * ranges.
	 * </p>
	 * 
	 * @param includeRanges
	 *            whether the start and length field of the StyleRanges should
	 *            be set.
	 * 
	 * @return the styles or an empty array if a LineStyleListener has been set.
	 * 
	 ...
	 * </p>
	 * <p>
	 * Note: Because the StyleRange includes the start and length, the same
	 * instance cannot occur multiple times in the array of styles. If the same
	 * style attributes, such as font and color, occur in multiple StyleRanges,
	 * <code>getStyleRanges(int, int, boolean)</code> can be used to get the
	 * styles without the ranges.
	 * </p>
	 * 
	 * @param start
	 *            the start offset of the style ranges to return
	 ...
	 *         returned <code>StyleRange</code> will have a starting offset >=
	 *         start and the last returned <code>StyleRange</code> will have an
	 *         ending offset <= start + length - 1
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_RANGE when start and/or length are
	 *                outside the widget content</li>
	 *                </ul>
	 */
	 ...
	 * Gets the top index.
	 * <p>
	 * The top index is the index of the fully visible line that is currently at
	 * the top of the widget or the topmost partially visible line if no line is
	 * fully visible. The top index changes when the widget is scrolled.
	 ...
	 * top of the widget. The text widget can be scrolled by pixels by dragging
	 * the scroll thumb so that a partial line may be displayed at the top the
	 * widget. The top pixel changes when the widget is scrolled. The top pixel
	 ...
	 * <p>
	 * <b>NOTE:</b> Does not return correct values for true italic fonts (vs.
	 * slanted fonts).
	 * </p>
	 * 
		...
		if (clickCount == 1) {
			...
			if (doubleClickEnabled) {
				...
				if (wordSelect) {
					...
					int max = blockSelection ? lineOffset + content.getLine(lineIndex).length() : content
							.getCharCount();
	 ...
	 * @param event
	 *            .newText text that is going to be inserted or empty String if
	 ...
	 * @param event
	 *            .replaceLineCount number of lines that are going to be
	 *            replaced
	 * @param event
	 *            .newLineCount number of new lines that are going to be
	 *            inserted
	 */
	...
	/**
	 * Called when the widget content is set programmatically, overwriting the
	 * old content. Resets the caret position, selection and scroll offsets.
	...
	/**
	 * Called when a traversal key is pressed. Allow tab next traversal to occur
	 * when the widget is in single line mode or in multi line and non-editable
	 * mode . When in editable multi line mode we want to prevent the tab
	 ...
	 * 
	 * @param action
	 *            one of the actions defined in ST.java
	 */
	...
	/**
	 * Returns true if an action should not be performed when block selection in
	 * active
	 */
	 ...
	 * writing order).
	 * 
	 * @return isMirrored true=the widget is right oriented, false=the widget is
	 *         left oriented
	 */
	boolean isMirrored() {
		return isMirrored;
	...
	/**
	 * Sends the specified verify event, replace/insert text as defined by the
	 * event and send a modify event.
	 * 
	 * @param event
	 ...
	 * caret offset. If the widget has the SWT.SINGLE style and the clipboard
	 * text contains more than one line, only the first line without line
	 * delimiters is inserted in the widget.
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 * completely painted.
	 * <p>
	 * Recalculates the content width for all lines in the bounds. When a
	 * <code>LineStyleListener</code> is used a redraw call is the only
	 * notification to the widget that styles have changed and that the content
	 ...
	 * processed, that area of the receiver will be painted. If the
	 * <code>all</code> flag is <code>true</code>, any children of the receiver
	 * which intersect with the specified area will also paint their
	 * intersecting areas. If the <code>all</code> flag is <code>false</code>,
	 * the children will not be painted.
	 * <p>
	 * Marks the content width of all lines in the specified rectangle as
	 * unknown. Recalculates the content width of all visible lines. When a
	 * <code>LineStyleListener</code> is used a redraw call is the only
	 * notification to the widget that styles have changed and that the content
	 * width may have changed.
	 * </p>
	 ...
	 * 
	 * @param start
	 *            offset of the first character to redraw
	 * @param length
	 *            number of characters to redraw
	 * @param clearBackground
	 *            true if the background should be cleared as part of the redraw
	 *            operation. If true, the entire redraw range will be cleared
	 *            before anything is redrawn. If the redraw range includes the
	 *            last character of a line (i.e., the entire line is redrawn)
	 *            the line is cleared all the way to the right border of the
	 *            widget. The redraw operation will be faster and smoother if
	 *            clearBackground is set to false. Whether or not the flag can
	 *            be set to false depends on the type of change that has taken
	 *            place. If font styles or background colors for the redraw
	 *            range have changed, clearBackground should be set to true. If
	 *            only foreground colors have changed for the redraw range,
	 ...
	 * effectively deletes the styles in the given range and then adds the the
	 * new styles.
	 * <p>
	 * Note: Because a StyleRange includes the start and length, the same
	 * instance cannot occur multiple times in the array of styles. If the same
	 * style attributes, such as font and color, occur in multiple StyleRanges,
	 * <code>setStyleRanges(int, int, int[], StyleRange[])</code> can be used to
	 * share styles and reduce memory usage.
	 * </p>
	 * <p>
	 ...
	 * @param length
	 *            length of the range to delete styles in
	 * @param ranges
	 *            StyleRange objects containing the new style information. The
	 *            ranges should not overlap and should be within the specified
	 *            start and length. The style rendering is undefined if the
	 *            ranges do overlap or are ill-defined. Must not be null.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_RANGE when either start or end is
	 *                outside the valid range (0 <= offset <= getCharCount())</li>
	...
	/**
	 * Replaces the given text range with new text. If the widget has the
	 * SWT.SINGLE style and "text" contains more than one line, only the first
	 * line is rendered but the text is stored unchanged. A subsequent call to
	 * getText will return the same text that was set. Note that only a single
	 * line of text should be set when the SWT.SINGLE style is used.
	 * <p>
	 ...
	 * remains selected
	 * <li>selection intersects replaced text: selection is cleared and caret is
	 * placed after inserted text
	 * </ul>
	 * </p>
	 * 
	 * @param start
	 *            offset of first character to replace
	 * @param length
	 ...
	 *                <li>ERROR_INVALID_RANGE when either start or end is
	 *                outside the valid range (0 <= offset <= getCharCount())</li>
	 *                <li>ERROR_INVALID_ARGUMENT when either start or end is
	 *                inside a multi byte line delimiter. Splitting a line
	 *                delimiter for example by inserting text in between the CR
	 *                and LF and deleting part of a line delimiter is not
	 *                supported</li>
	 *                <li>ERROR_NULL_ARGUMENT when string is null</li>
	 *                </ul>
	 */
	...
	/**
	 * Resets the caret position, selection and scroll offsets. Recalculate the
	 * content width and scroll bars. Redraw the widget.
	 */
	void reset() {
		ScrollBar verticalBar = getVerticalBar();
		ScrollBar horizontalBar = getHorizontalBar();
	...
	/**
	 * Returns a StyledTextEvent that can be used to request data such as styles
	 * and background color for a line.
	 * <p>
	 * The specified line may be a visual (wrapped) line if in word wrap mode.
	 * The returned object will always be for a logical (unwrapped) line.
	 * </p>
	 * 
	 * @param lineOffset
	 *            offset of the line. This may be the offset of a visual line if
	 *            the widget is in word wrap mode.
	 * @param line
	 *            line text. This may be the text of a visual line if the widget
	 *            is in word wrap mode.
	 * @return StyledTextEvent that can be used to request line data for the
	 *         given line.
	 */
	...
	/**
	 * Sets the alignment of the widget. The argument should be one of
	 * <code>SWT.LEFT</code>, <code>SWT.CENTER</code> or <code>SWT.RIGHT</code>.
	 * The alignment applies for all lines. </p>
	 * <p>
	 * Note that if <code>SWT.MULTI</code> is set, then <code>SWT.WRAP</code>
	 * must also be set in order to stabilize the right edge before setting
	 * alignment.
	 * </p>
	 * 
	 * @param alignment
	 ...
	 *            true=enable block selection, false=disable block selection
	 * 
	 * @since 3.5
	 */
	...
	/**
	 * Sets the block selection bounds. The bounds is relative to the upper left
	 * corner of the document.
	 * 
	 * @param rect
	 *            the new bounds for the block selection
	 * 
	 ...
	 * 
	 * @param x
	 *            the new x coordinate for the block selection
	 * @param y
	 *            the new y coordinate for the block selection
	 * @param width
	 *            the new width for the block selection
	 * @param height
	...
	/**
	 * Sets the BIDI coloring mode. When true the BIDI text display algorithm is
	 * applied to segments of text that are the same color.
	 * 
	 * @param mode
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT when either the start or the
	 *                end of the selection range is inside a multi byte line
	 *                delimiter (and thus neither clearly in front of or after
	 *                the line delimiter)
	 *                </ul>
	 */
	 ...
	 * Overridden to handle the null case since the StyledText widget uses an
	 * ibeam as its default cursor.
	 * 
	...
	/**
	 * Sets whether the widget implements double click mouse behavior. </p>
	 * 
	 * @param enable
	...
	/**
	 * Sets the horizontal scroll offset relative to the start of the line. Do
	 * nothing if there is no text set.
	 * <p>
	 * <b>NOTE:</b> The horizontal index is reset to 0 when new text is set in
	 * the widget.
	 * </p>
	 * 
	 * @param offset
	 *            horizontal scroll offset relative to the start of the line,
	 *            measured in character increments starting at 0, if equal to 0
	 *            the content is not scrolled, if > 0 = the content is scrolled.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * <p>
	 * It is the amount of blank space, in pixels, at the beginning of each
	 * line. When a line wraps in several lines only the first one is indented.
	 * </p>
	 * 
	 * @param indent
	 *            the new indent
	 ...
	 * <p>
	 * One action can be associated with N keys. However, each key can only have
	 * one action (key:action is N:1 relation).
	 * </p>
	 * 
	 * @param key
	 *            a key code defined in SWT.java or a character. Optionally ORd
	 ...
	 *            SWT.SHIFT, SWT.ALT, SWT.COMMAND) makes sense.
	 * @param action
	 *            one of the predefined actions defined in ST.java. Use SWT.NULL
	...
	/**
	 * Sets the alignment of the specified lines. The argument should be one of
	 * <code>SWT.LEFT</code>, <code>SWT.CENTER</code> or <code>SWT.RIGHT</code>.
	 * <p>
	 * <p>
	 * Note that if <code>SWT.MULTI</code> is set, then <code>SWT.WRAP</code>
	 ...
	 * </p>
	 * Should not be called if a LineStyleListener has been set since the
	 * listener maintains the line attributes. </p>
	 * <p>
	 * All line attributes are maintained relative to the line text, not the
	 * line index that is specified in this method call. During text changes,
	 * when entire lines are inserted or removed, the line attributes that are
	 * associated with the lines after the change will "move" with their
	 * respective text. An entire line is defined as extending from the first
	 * character on a line to the last and including the line delimiter.
	 * </p>
	 * <p>
	 * When two lines are joined by deleting a line delimiter, the top line
	 * attributes take precedence and the attributes of the bottom line are
	 * deleted. For all other text changes line attributes will remain
	 * unchanged.
	 * 
	 * @param startLine
	 *            first line the alignment is applied to, 0 based
	 * @param lineCount
	 *            number of lines the alignment applies to.
	 * @param alignment
	 *            line alignment
	 * 
	 * @exception SWTException
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_ARGUMENT when the specified line range
	 *                is invalid</li>
	 *                </ul>
	 ...
	 * <p>
	 * The background color is drawn for the width of the widget. All line
	 * background colors are discarded when setText is called. The text
	 * background color if defined in a StyleRange overlays the line background
	 * color.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param startLine
	 *            first line the color is applied to, 0 based
	 * @param lineCount
	 *            number of lines the color applies to.
	 * @param background
	 ...
	 * <p>
	 * Should not be called if a LineStyleListener has been set since the
	 * listener maintains the line attributes.
	 * </p>
	 * <p>
	 * All line attributes are maintained relative to the line text, not the
	 * line index that is specified in this method call. During text changes,
	 ...
	 * 
	 * @param startLine
	 *            first line the bullet is applied to, 0 based
	 * @param lineCount
	 ...
	 * 
	 * @param startLine
	 *            first line the indent is applied to, 0 based
	 * @param lineCount
	 *            number of lines the indent applies to.
	 * @param indent
	 *            line indent
	 * 
	 * @exception SWTException
	 ...
	 * 
	 * @param startLine
	 *            first line the justify is applied to, 0 based
	 * @param lineCount
	 *            number of lines the justify applies to.
	 * @param justify
	...
	/**
	 * Sets the line spacing of the widget. The line spacing applies for all
	 * lines.
	 * 
	 * @param lineSpacing
	 *            the line spacing
	...
	void setScrollBars(boolean vertical) {
		...
		if (horizontalBar != null && horizontalBar.getVisible()) {
			...
			if (clientAreaWidth < maximum) {
				...
				horizontalBar.setValues(horizontalBar.getSelection(), horizontalBar.getMinimum(), inactive, inactive,
	 ...
	 * positions. In a text widget that contains N characters, there are N+1
	 * caret positions, ranging from 0..N
	 * </p>
	 * 
	 * @param point
	 *            x=selection start offset, y=selection end offset The caret
	...
	/**
	 * Sets the receiver's selection foreground color to the color specified by
	 * the argument, or to the default system color for the control if the
	 * argument is null.
	 * <p>
	 * Note that this is a <em>HINT</em>. Some platforms do not allow the
	 * application to change the selection foreground color.
	 * </p>
	 * 
	 * @param color
	 *            the new color (or null)
	 ...
	 * 
	 * @param start
	 *            offset of the first selected character, start >= 0 must be
	 *            true.
	 * @param length
	 *            number of characters to select, 0 <= start + length <=
	 *            getCharCount() must be true. A negative length places the
	 *            caret at the selection start.
	 * @param sendEvent
	 *            a Selection event is sent when set to true and when the
	 *            selection is reset.
	 */
	void setSelection(int start, int length, boolean sendEvent, boolean doBlock) {
		...
		// different?
		if (selection.x != start || selection.y != end || length > 0 && selectionAnchor != selection.x || length < 0
				&& selectionAnchor != selection.y) {
	 ...
	 * <p>
	 * The new selection may not be visible. Call showSelection to scroll the
	 * selection into view. A negative length places the caret at the visual
	 ...
	 * Existing style ranges are adjusted if they partially overlap with the new
	 * style. To clear an individual style, call setStyleRange with a StyleRange
	 * that has null attributes.
	 * </p>
	 * <p>
	 * Should not be called if a LineStyleListener has been set since the
	 * listener maintains the styles.
	 ...
	 * 
	 * @param range
	 *            StyleRange object containing the style information. Overwrites
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_INVALID_RANGE when the style range is outside
	 *                the valid range (> getCharCount())</li>
	 *                </ul>
	 */
	 ...
	 * The ranges array contains start and length pairs. Each pair refers to the
	 * corresponding style in the styles array. For example, the pair that
	 * starts at ranges[n] with length ranges[n+1] uses the style at
	 * styles[n/2]. The range fields within each StyleRange are ignored. If
	 * ranges or styles is null, the specified range is cleared.
	 * </p>
	 * <p>
	 ...
	 *            length of the range to delete styles in
	 * @param ranges
	 *            the array of ranges. The ranges must not overlap and must be
	 *            in order.
	 * @param styles
	 *            the array of StyleRanges. The range fields within the
	 *            StyleRange are unused.
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_NULL_ARGUMENT when an element in the styles
	 *                array is null</li>
	 *                <li>ERROR_INVALID_RANGE when the number of ranges and
	 *                style do not match (ranges.length * 2 == styles.length)</li>
	 *                <li>ERROR_INVALID_RANGE when a range is outside the valid
	 *                range (> getCharCount() or less than zero)</li>
	 ...
	 * <p>
	 * All styles in the widget will be replaced with the given set of ranges
	 * and styles. The ranges array contains start and length pairs. Each pair
	 * refers to the corresponding style in the styles array. For example, the
	 * pair that starts at ranges[n] with length ranges[n+1] uses the style at
	 * styles[n/2]. The range fields within each StyleRange are ignored. If
	 * either argument is null, the styles are cleared.
	 * </p>
	 * <p>
	 * Note: It is expected that the same instance of a StyleRange will occur
	 * multiple times within the styles array, reducing memory usage.
	 ...
	 * 
	 * @param ranges
	 *            StyleRange objects containing the style information. The
	 *            ranges should not overlap. The style rendering is undefined if
	 *            the ranges do overlap. Must not be null. The styles need to be
	 *            in order.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 *                <li>ERROR_INVALID_RANGE when the last of the style ranges
	 *                is outside the valid range (> getCharCount())</li>
	 *                </ul>
	 * 
	 * @see #setStyleRanges(int[], StyleRange[])
	 */
	...
	/**
	 * Sets the widget content. If the widget has the SWT.SINGLE style and
	 * "text" contains more than one line, only the first line is rendered but
	 * the text is stored unchanged. A subsequent call to getText will return
	 * the same text that was set.
	 * <p>
	 ...
	 *            that were set using StyledText API are discarded. The current
	 *            selection is also discarded.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * <p>
	 * The text limit specifies the amount of text that the user can type into
	 * the widget.
	 * </p>
	 * 
	 * @param limit
	 ...
	 * <p>
	 * The top index is the index of the line that is currently at the top of
	 * the widget. The top index changes when the widget is scrolled. Indexing
	 * starts from zero. Note: The top index is reset to 0 when new text is set
	 * in the widget.
	 * </p>
	 * 
	 * @param topIndex
	 *            new top index. Must be between 0 and getLineCount() - fully
	 *            visible lines per page. If no lines are fully visible the
	 *            maximum value is getLineCount() - 1. An out of range index
	 ...
	 * widget is scrolled so that the given pixel position is at the top. The
	 * top index is adjusted to the corresponding top line. Note: The top pixel
	 * is reset to 0 when new text is set in the widget.
	 * </p>
	 * 
	 * @param pixel
	 *            new top pixel offset. Must be between 0 and (getLineCount() -
	 *            visible lines per page) / getLineHeight()). An out of range
	 *            offset will be adjusted accordingly.
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                disposed</li>
	 ...
	 * <p>
	 * The end of the selection will be scrolled into view. Note that if a
	 * right-to-left selection exists, the end of the selection is the visual
	 * beginning of the selection (i.e., where the caret is located).
	 * </p>
	 * 
	 * @exception SWTException
	 *                <ul>
	 ...
	 * reset and the caret moved to the end of the new text. If the selection is
	 * behind the replaced text it is moved so that the same text remains
	 * selected. If the selection is before the replaced text it is left
	 * unchanged.
	 * </p>
	 * 
	 * @param startOffset
	 *            offset of the text change

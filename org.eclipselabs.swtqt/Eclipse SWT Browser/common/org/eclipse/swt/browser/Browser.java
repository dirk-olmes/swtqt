 ...
 * <dd>MOZILLA</dd>
 * <dt><b>Events:</b></dt>
 * <dd>CloseWindowListener, LocationListener, OpenWindowListener,
 ...
 * </p>
 * 
 * @see <a href="http://www.eclipse.org/swt/snippets/#browser">Browser
 *      snippets</a>
 * @see <a href="http://www.eclipse.org/swt/examples.php">SWT Examples:
 *      ControlExample, BrowserExample</a>
...
public class Browser extends Composite {
	...
	public Browser(Composite parent, int style) {
		...
		if ((style & SWT.MOZILLA) != 0) {
			...
			} else if ("qt".equals(platform)) { //$NON-NLS-1$
				className = "org.eclipse.swt.browser.QtWebkit"; //$NON-NLS-1$
			} else {
				dispose();
	...
	static Composite checkParent(Composite parent) {
		...
		/*
		 * Note. Mozilla provides all IM support needed for text input in web
		 * pages. If SWT creates another input method context for the widget it
		 * will cause indeterminate results to happen (hangs and crashes). The
		 * fix is to prevent SWT from creating an input method context for the
		 * Browser widget.
		 */
	...
	static int checkStyle(int style) {
		...
		if ("win32".equals(platform)) { //$NON-NLS-1$
			/*
			 * For IE on win32 the border is supplied by the embedded browser,
			 * so remove the style so that the parent Composite will not draw a
			 * second border.
			 */
	...
	/**
	 * Returns the value of a cookie that is associated with a URL. Note that
	 * cookies are shared amongst all Browser instances.
	 * 
	 * @param name
	 *            the cookie name
	 * @param url
	 *            the URL that the cookie is associated with
	...
	/**
	 * Sets a cookie on a URL. Note that cookies are shared amongst all Browser
	 * instances.
	 * 
	 * The <code>value</code> parameter must be a cookie header string that
	 * complies with <a href="http://www.ietf.org/rfc/rfc2109.txt">RFC
	 * 2109</code>. The value is passed through to the native browser unchanged.
	 * <p>
	 * Example value strings: <code>foo=bar</code> (basic session cookie)
	 * <code>foo=bar; path=/; domain=.eclipse.org</code> (session cookie)
	 * <code>foo=bar; expires=Thu, 01-Jan-2030 00:00:01 GMT</code> (persistent
	 * cookie) <code>foo=; expires=Thu, 01-Jan-1970 00:00:01 GMT</code> (deletes
	 * cookie <code>foo</code>)
	 * 
	 * @param value
	 *            the cookie value
	 * @param url
	 *            the URL to associate the cookie with
	 * @return <code>true</code> if the cookie was successfully set and
	 *         <code>false</code> otherwise
	 * 
	 * @exception IllegalArgumentException
	 *                <ul>
	 *                <li>ERROR_NULL_ARGUMENT - if the value is null</li> <li>
	 *                ERROR_NULL_ARGUMENT - if the url is null</li>
	 *                </ul>
	 * 
	 * @since 3.5
	 */
	 ...
	 * <p>
	 * Executes a script containing javascript commands in the context of the
	 * current document. If document-defined functions or properties are
	 * accessed by the script then this method should not be invoked until the
	 * document has finished loading (<code>ProgressListener.completed()</code>
	 ...
	 * <p>
	 * Evaluates a script containing javascript commands in the context of the
	 * current document. If document-defined functions or properties are
	 * accessed by the script then this method should not be invoked until the
	 * document has finished loading (<code>ProgressListener.completed()</code>
	 * gives notification of this).
	 ...
	 * representation of the value is returned. The supported javascript -> java
	 * mappings are:
	 * <ul>
	 ...
	 * unsupported type, or if evaluating the script causes a javascript error
	 * to be thrown.
	 * 
	 * @param script
	 *            the script with javascript commands
	 ...
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_FAILED_EVALUATE when the script evaluation
	 *                causes a javascript error to be thrown</li> <li>
	 *                ERROR_INVALID_RETURN_VALUE when the script returns a value
	 *                of unsupported type</li> <li>ERROR_THREAD_INVALID_ACCESS
	 *                when called from the wrong thread</li> <li>
	...
	/**
	 * Returns the type of native browser being used by this instance. Examples:
	 * "mozilla", "ie", "safari", "voyager"
	 * 
	...
	public int getStyle() {
		/*
		 * If SWT.BORDER was specified at creation time then getStyle() should
		 * answer it even though it is removed for IE on win32 in checkStyle().
		 */
		return super.getStyle() | userStyle & SWT.BORDER;
	}

	/**
	 * Returns a string with HTML that represents the content of the current
	 * page.
	 * 
	 * @return HTML representing the current page or an empty
	 *         <code>String</code> if this is empty
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_THREAD_INVALID_ACCESS when called from the wrong
	...
	/**
	 * Returns the JavaXPCOM <code>nsIWebBrowser</code> for the receiver, or
	 * <code>null</code> if it is not available. In order for an
	 * <code>nsIWebBrowser</code> to be returned all of the following must be
	 * true:
	 * <ul>
	 * <li>the receiver's style must be <code>SWT.MOZILLA</code></li>
	 * <li>the classes from JavaXPCOM &gt;= 1.8.1.2 must be resolvable at
	 * runtime</li>
	 * <li>the version of the underlying XULRunner must be &gt;= 1.8.1.2</li>
	 * </ul>
	 * 
	 * @return the receiver's JavaXPCOM <code>nsIWebBrowser</code> or
	 *         <code>null</code>
	 * 
	 * @since 3.3
	 */
	 ...
	 * viewed in the receiver. Note that setting this value does not affect the
	 * running of javascript in the current page.
	 * 
	 * @param enabled
	 *            the receiver's new javascript enabled state
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been

...
public class OS extends C {
	...
	public static final int COMCTL32_MAJOR = 6;
	//, COMCTL32_MINOR, COMCTL32_VERSION;
	//public static final int SHELL32_MAJOR, SHELL32_MINOR, SHELL32_VERSION;

	public static final String NO_MANIFEST = "org.eclipse.swt.internal.win32.OS.NO_MANIFEST"; //$NON-NLS-1$
	...
	static {
		/*
		 * Try the UNICODE version of GetVersionEx first and then the ANSI
		 * version. The UNICODE version is present on all versions of Windows
		 * but is not implemented on Win95/98/ME.
		 * 
		 * NOTE: The value of OSVERSIONINFO.sizeof cannot be static final
		 * because it relies on the Windows platform version to be initialized
		 * and IsUnicode has not been calculated. It must be initialized here,
		 * after the platform is determined in order for the value to be
		 * correct.
		 */
		...
		IsUnicode = false; // just to have a value

	}
	/*
	 * NOTE: There is a bug in JVM 1.2 where loading a class with a large number
	 * of constants causes a segment fault to occur sometime later after the
	 * class is loaded. The fix is to break the class up into a hierarchy of
	 * classes that each contain a smaller number of constants. This fix is not
	 * necessary at this time but is required when all constants are
	 * uncommented. We have not done the research to determine the limit.
	 */
	...
	public static final int QS_ALLINPUT = QS_MOUSEMOVE | QS_MOUSEBUTTON | QS_KEY | QS_POSTMESSAGE | QS_TIMER | QS_PAINT
			| QS_SENDMESSAGE;
	...
	public static final int AssocQueryString(int flags, int str, TCHAR pszAssoc, TCHAR pszExtra, TCHAR pszOut,
			int[] pcchOut) {
		if (IsUnicode) {
	...
	public static final int /* long */CallWindowProc(int /* long */lpPrevWndFunc, int /* long */hWnd, int Msg,
	...
	public static final int /* long */CreateDC(TCHAR lpszDriver, TCHAR lpszDevice, int /* long */lpszOutput,
			int /* long */lpInitData) {
		if (IsUnicode) {
	...
	public static final int /* long */CreateEnhMetaFile(int /* long */hdcRef, TCHAR lpFilename, RECT lpRect,
			TCHAR lpDescription) {
		if (IsUnicode) {
	...
	public static final boolean CreateProcess(int /* long */lpApplicationName, int /* long */lpCommandLine,
			int /* long */lpProcessAttributes, int /* long */lpThreadAttributes, boolean bInheritHandles,
			int dwCreationFlags, int /* long */lpEnvironment, int /* long */lpCurrentDirectory,
	...
	public static final int /* long */CreateWindowEx(int dwExStyle, TCHAR lpClassName, TCHAR lpWindowName, int dwStyle,
			int X, int Y, int nWidth, int nHeight, int /* long */hWndParent, int /* long */hMenu,
			int /* long */hInstance, CREATESTRUCT lpParam) {
		if (IsUnicode) {
			char[] lpClassName1 = lpClassName == null ? null : lpClassName.chars;
			char[] lpWindowName1 = lpWindowName == null ? null : lpWindowName.chars;
			return CreateWindowExW(dwExStyle, lpClassName1, lpWindowName1, dwStyle, X, Y, nWidth, nHeight, hWndParent,
		...
		return CreateWindowExA(dwExStyle, lpClassName1, lpWindowName1, dwStyle, X, Y, nWidth, nHeight, hWndParent,
				hMenu, hInstance, lpParam);
	}
	...
	public static final boolean EnumSystemLanguageGroups(int /* long */pLangGroupEnumProc, int dwFlags,
			int /* long */lParam) {
		if (IsUnicode) {
	...
	public static final int GetCharacterPlacement(int /* long */hdc, TCHAR lpString, int nCount, int nMaxExtent,
			GCP_RESULTS lpResults, int dwFlags) {
		if (IsUnicode) {
	...
	public static final int GetProfileString(TCHAR lpAppName, TCHAR lpKeyName, TCHAR lpDefault, TCHAR lpReturnedString,
			int nSize) {
		if (IsUnicode) {
	...
	public static final boolean InternetGetCookie(TCHAR lpszUrl, TCHAR lpszCookieName, TCHAR lpszCookieData,
			int[] lpdwSize) {
		if (IsUnicode) {
	...
	public static final boolean PeekMessage(MSG lpMsg, int /* long */hWnd, int wMsgFilterMin, int wMsgFilterMax,
			int wRemoveMsg) {
		if (IsUnicode) {
	...
	public static final int RegEnumKeyEx(int /* long */hKey, int dwIndex, TCHAR lpName, int[] lpcName, int[] lpReserved,
			TCHAR lpClass, int[] lpcClass, FILETIME lpftLastWriteTime) {
		if (IsUnicode) {
	...
	public static final int RegOpenKeyEx(int /* long */hKey, TCHAR lpSubKey, int ulOptions, int samDesired,
			int /* long */[] phkResult) {
		if (IsUnicode) {
	...
	public static final int RegQueryValueEx(int /* long */hKey, TCHAR lpValueName, int /* long */lpReserved,
			int[] lpType, TCHAR lpData, int[] lpcbData) {
		if (IsUnicode) {
	...
	public static final int /* long */SetWindowsHookEx(int idHook, int /* long */lpfn, int /* long */hMod,
			int dwThreadId) {
		if (IsUnicode) {
	...
	public static int /* long */SHGetFileInfo(TCHAR pszPath, int dwFileAttributes, SHFILEINFO psfi, int cbFileInfo,
			int uFlags) {
		if (IsUnicode) {
	...
	public static final native int AssocQueryStringA(int flags, int str, byte[] pszAssoc, byte[] pszExtra,
			byte[] pszOut, int[] pcchOut);

	/** @method flags=dynamic */
	public static final native int AssocQueryStringW(int flags, int str, char[] pszAssoc, char[] pszExtra,
			char[] pszOut, int[] pcchOut);
	 ...
	 *            cast=(HDC)
	 */
	public static final native boolean BitBlt(int /* long */hdcDest, int nXDest, int nYDest, int nWidth, int nHeight,
			int /* long */hdcSrc, int nXSrc, int nYSrc, int dwRop);
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native int /* long */CallNextHookEx(int /* long */hhk, int nCode, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(HWND)
	 */
	public static final native int /* long */CallWindowProcW(int /* long */lpPrevWndFunc, int /* long */hWnd, int Msg,
			int /* long */wParam, int /* long */lParam);
	 ...
	 *            cast=(HWND)
	 */
	public static final native int /* long */CallWindowProcA(int /* long */lpPrevWndFunc, int /* long */hWnd, int Msg,
			int /* long */wParam, int /* long */lParam);
	 ...
	 *            cast=(LPVOID *)
	 */
	public static final native int CoCreateInstance(byte[] rclsid, int /* long */pUnkOuter, int dwClsContext,
			byte[] riid, int /* long */[] ppv);
	 ...
	 *            cast=(HANDLE)
	 */
	public static final native int /* long */CopyImage(int /* long */hImage, int uType, int cxDesired, int cyDesired,
			int fuFlags);

	/**
	 * @param lpvBits
	 *            cast=(CONST VOID *),flags=no_out critical
	 */
	public static final native int /* long */CreateBitmap(int nWidth, int nHeight, int cPlanes, int cBitsPerPel,
			byte[] lpvBits);
	 ...
	 *            cast=(CONST VOID *),flags=no_out critical
	 */
	public static final native int /* long */CreateCursor(int /* long */hInst, int xHotSpot, int yHotSpot,
	 ...
	 *            cast=(CONST DEVMODEW *)
	 */
	public static final native int /* long */CreateDCW(char[] lpszDriver, char[] lpszDevice, int /* long */lpszOutput,
			int /* long */lpInitData);
	 ...
	 *            cast=(CONST DEVMODE *)
	 */
	public static final native int /* long */CreateDCA(byte[] lpszDriver, byte[] lpszDevice, int /* long */lpszOutput,
			int /* long */lpInitData);
	 ...
	 *            cast=(HDC)
	 * @param pbmi
	 *            cast=(BITMAPINFO *),flags=no_out critical
	 * @param ppvBits
	 *            cast=(VOID **),flags=no_in critical
	 * @param hSection
	 *            cast=(HANDLE)
	 */
	public static final native int /* long */CreateDIBSection(int /* long */hdc, byte[] pbmi, int iUsage,
			int /* long */[] ppvBits, int /* long */hSection, int dwOffset);
	 ...
	 *            cast=(HANDLE)
	 */
	public static final native int /* long */CreateDIBSection(int /* long */hdc, int /* long */pbmi, int iUsage,
			int /* long */[] ppvBits, int /* long */hSection, int dwOffset);
	 ...
	 *            cast=(LPCWSTR)
	 */
	public static final native int /* long */CreateEnhMetaFileW(int /* long */hdcRef, char[] lpFilename, RECT lpRect,
			char[] lpDescription);
	 ...
	 *            cast=(LPCSTR)
	 */
	public static final native int /* long */CreateEnhMetaFileA(int /* long */hdcRef, byte[] lpFilename, RECT lpRect,
			byte[] lpDescription);
	 ...
	 */
	public static final native boolean CreateProcessW(int /* long */lpApplicationName, int /* long */lpCommandLine,
			int /* long */lpProcessAttributes, int /* long */lpThreadAttributes, boolean bInheritHandles,
			int dwCreationFlags, int /* long */lpEnvironment, int /* long */lpCurrentDirectory,
			STARTUPINFO lpStartupInfo, PROCESS_INFORMATION lpProcessInformation);
	 ...
	 *            cast=(LPPROCESS_INFORMATION)
	 */
	public static final native boolean CreateProcessA(int /* long */lpApplicationName, int /* long */lpCommandLine,
			int /* long */lpProcessAttributes, int /* long */lpThreadAttributes, boolean bInheritHandles,
			int dwCreationFlags, int /* long */lpEnvironment, int /* long */lpCurrentDirectory,
			STARTUPINFO lpStartupInfo, PROCESS_INFORMATION lpProcessInformation);
	 ...
	 * @param hGlobal
	 *            cast=(HGLOBAL)
	 * @param fDeleteOnRelease
	 *            cast=(BOOL)
	 * @param ppstm
	 *            cast=(LPSTREAM *)
	 */
	public static final native int CreateStreamOnHGlobal(int /* long */hGlobal, boolean fDeleteOnRelease,
			int /* long */[] ppstm);
	 ...
	 *            cast=(HINSTANCE)
	 */
	public static final native int /* long */CreateWindowExW(int dwExStyle, char[] lpClassName, char[] lpWindowName,
			int dwStyle, int X, int Y, int nWidth, int nHeight, int /* long */hWndParent, int /* long */hMenu,
			int /* long */hInstance, CREATESTRUCT lpParam);
	 ...
	 *            cast=(HINSTANCE)
	 */
	public static final native int /* long */CreateWindowExA(int dwExStyle, byte[] lpClassName, byte[] lpWindowName,
			int dwStyle, int X, int Y, int nWidth, int nHeight, int /* long */hWndParent, int /* long */hMenu,
			int /* long */hInstance, CREATESTRUCT lpParam);
	 ...
	 *            cast=(const RECT *)
	 */
	public static final native int DrawThemeBackground(int /* long */hTheme, int /* long */hdc, int iPartId,
			int iStateId, RECT pRect, RECT pClipRect);

	/** @method flags=dynamic */
	public static final native int DrawThemeText(int /* long */hTheme, int /* long */hdc, int iPartId, int iStateId,
			char[] pszText, int iCharCount, int dwTextFlags, int dwTextFlags2, RECT pRect);

	/**
	 * @method flags=dynamic
	 * @param pLangGroupEnumProc
	 *            cast=(LANGUAGEGROUP_ENUMPROCW)
	 * @param lParam
	 *            cast=(LONG_PTR)
	 */
	public static final native boolean EnumSystemLanguageGroupsW(int /* long */pLangGroupEnumProc, int dwFlags,
			int /* long */lParam);

	/**
	 * @method flags=dynamic
	 * @param pLangGroupEnumProc
	 *            cast=(LANGUAGEGROUP_ENUMPROCA)
	 * @param lParam
	 *            cast=(LONG_PTR)
	 */
	public static final native boolean EnumSystemLanguageGroupsA(int /* long */pLangGroupEnumProc, int dwFlags,
			int /* long */lParam);
	 ...
	 *            cast=(LPVOID)
	 */
	public static final native boolean EnumEnhMetaFile(int /* long */hdc, int /* long */hemf,
			int /* long */lpEnhMetaFunc, int /* long */lpData, RECT lpRect);

	/**
	 * @param hdc
	 *            cast=(HDC)
	 */
	public static final native int ExcludeClipRect(int /* long */hdc, int nLeftRect, int nTopRect, int nRightRect,
			int nBottomRect);
	 ...
	 *            cast=(CONST DWORD *)
	 */
	public static final native int /* long */ExtCreatePen(int dwPenStyle, int dwWidth, LOGBRUSH lplb,
			int dwStyleCount, int[] lpStyle);
	 ...
	 *            flags=no_out
	 * @param lpString
	 *            cast=(LPWSTR),flags=no_out critical
	 * @param lpDx
	 *            cast=(CONST INT *),flags=no_out critical
	 */
	public static final native boolean ExtTextOutW(int /* long */hdc, int X, int Y, int fuOptions, RECT lprc,
			char[] lpString, int cbCount, int[] lpDx);
	 ...
	 *            cast=(HDC)
	 * @param lpabc
	 *            cast=(LPABC),flags=no_in critical
	 */
	 ...
	 *            cast=(LPGCP_RESULTSW)
	 */
	public static final native int GetCharacterPlacementW(int /* long */hdc, char[] lpString, int nCount, int nMaxExtent,
			GCP_RESULTS lpResults, int dwFlags);
	 ...
	 *            cast=(HDC)
	 * @param lpString
	 *            cast=(LPSTR),flags=no_out critical
	 */
	public static final native int GetCharacterPlacementA(int /* long */hdc, byte[] lpString, int nCount, int nMaxExtent,
			GCP_RESULTS lpResults, int dwFlags);
	 ...
	 *            cast=(HDC)
	 * @param lpBuffer
	 *            cast=(LPINT),flags=no_in critical
	 */
	 ...
	 *            cast=(HDC)
	 * @param pColors
	 *            cast=(RGBQUAD *),flags=no_in critical
	 */
	 ...
	 *            cast=(HMENU)
	 * @param lpmii
	 *            cast=(LPMENUITEMINFOW)
	 */
	public static final native boolean GetMenuItemInfoW(int /* long */hMenu, int uItem, boolean fByPosition,
			MENUITEMINFO lpmii);
	 ...
	 *            cast=(HMENU)
	 */
	public static final native boolean GetMenuItemInfoA(int /* long */hMenu, int uItem, boolean fByPosition,
			MENUITEMINFO lpmii);
	...
	/** @method flags=dynamic */
	public static final native int GetThemeTextExtent(int /* long */hTheme, int /* long */hdc, int iPartId, int iStateId,
			char[] pszText, int iCharCount, int dwTextFlags, RECT pBoundingRect, RECT pExtentRect);
	 ...
	 *            cast=(LPWSTR)
	 */
	public static final native int GetProfileStringW(char[] lpAppName, char[] lpKeyName, char[] lpDefault,
	 ...
	 *            cast=(LPSTR)
	 */
	public static final native int GetProfileStringA(byte[] lpAppName, byte[] lpKeyName, byte[] lpDefault,
	 ...
	 *            cast=(UINT)
	 * @param lppe
	 *            cast=(LPPALETTEENTRY),flags=no_in critical
	 */
	public static final native int GetSystemPaletteEntries(int /* long */hdc, int iStartIndex, int nEntries,
			byte[] lppe);
	 ...
	 *            flags=no_in
	 */
	public static final native boolean GetTextExtentPoint32W(int /* long */hdc, char[] lpString, int cbString,
			SIZE lpSize);
	...
	/** @method flags=dynamic */
	public static final native int GetThemeInt(int /* long */hTheme, int iPartId, int iStateId, int iPropId,
			int[] piVal);

	/** @method flags=dynamic */
	public static final native int GetThemeBackgroundContentRect(int /* long */hTheme, int /* long */hdc, int iPartId,
			int iStateId, RECT pBoundingRect, RECT pContentRect);
	 ...
	 *            flags=no_out
	 */
	public static final native int GetThemeBackgroundExtent(int /* long */hTheme, int /* long */hdc, int iPartId,
			int iStateId, RECT pContentRect, RECT pExtentRect);

	/** @method flags=dynamic */
	public static final native int GetThemePartSize(int /* long */hTheme, int /* long */hdc, int iPartId, int iStateId,
			RECT prc, int eSize, SIZE psz);
	 ...
	 *            flags=struct
	 */
	public static final native int HitTestThemeBackground(int /* long */hTheme, int /* long */hdc, int iPartId,
			int iStateId, int dwOptions, RECT pRect, int /* long */hrgn, POINT ptTest, short[] pwHitTestCode);
	 ...
	 *            cast=(UINT)
	 */
	public static final native boolean ImageList_Draw(int /* long */himl, int i, int /* long */hdcDst, int x, int y,
			int fStyle);
	 ...
	 *            cast=(HBITMAP)
	 */
	public static final native boolean ImageList_Replace(int /* long */himl, int i, int /* long */hbmImage,
			int /* long */hbmMask);
	 ...
	 *            cast=(LPMENUITEMINFOW)
	 */
	public static final native boolean InsertMenuItemW(int /* long */hMenu, int uItem, boolean fByPosition,
			MENUITEMINFO lpmii);
	 ...
	 *            cast=(HMENU)
	 */
	public static final native boolean InsertMenuItemA(int /* long */hMenu, int uItem, boolean fByPosition,
			MENUITEMINFO lpmii);

	/**
	 * @param lpszUrl
	 *            cast=(LPCTSTR)
	 * @param lpszCookieName
	 *            cast=(LPCTSTR)
	 * @param lpszCookieData
	 *            cast=(LPSTR)
	 * @param lpdwSize
	 *            cast=(LPDWORD)
	 */
	public static final native boolean InternetGetCookieA(byte[] lpszUrl, byte[] lpszCookieName, byte[] lpszCookieData,
			int[] lpdwSize);
	 ...
	 *            cast=(LPDWORD)
	 */
	public static final native boolean InternetGetCookieW(char[] lpszUrl, char[] lpszCookieName, char[] lpszCookieData,
			int[] lpdwSize);
	 ...
	 *            cast=(LPVOID)
	 */
	public static final native boolean InternetSetOption(int /* long */hInternet, int dwOption, int /* long */lpBuffer,
			int dwBufferLength);
	 ...
	 *            cast=(HDC)
	 */
	public static final native int IntersectClipRect(int /* long */hdc, int nLeftRect, int nTopRect, int nRightRect,
			int nBottomRect);
	 ...
	 *            cast=(LPPOINT)
	 */
	public static final native int MapWindowPoints(int /* long */hWndFrom, int /* long */hWndTo, POINT lpPoints,
			int cPoints);
	...
	/**
	 * @param Destination
	 *            cast=(PVOID),flags=no_in critical
	 * @param SourcePtr
	 *            cast=(CONST VOID *)
	 */
	 ...
	 *            cast=(LPCSTR),flags=no_out critical
	 * @param lpWideCharStr
	 *            cast=(LPWSTR),flags=no_in critical
	 */
	public static final native int MultiByteToWideChar(int CodePage, int dwFlags, byte[] lpMultiByteStr,
	 ...
	 *            cast=(LPWSTR),flags=no_in critical
	 */
	public static final native int MultiByteToWideChar(int CodePage, int dwFlags, int /* long */lpMultiByteStr,
			int cchMultiByte, char[] lpWideCharStr, int cchWideChar);
	 ...
	 *            cast=(HWND)
	 */
	public static final native boolean PeekMessageW(MSG lpMsg, int /* long */hWnd, int wMsgFilterMin,
	 ...
	 *            cast=(HWND)
	 */
	public static final native boolean PeekMessageA(MSG lpMsg, int /* long */hWnd, int wMsgFilterMin,
			int wMsgFilterMax, int wRemoveMsg);
	 ...
	 *            cast=(HDC)
	 * @param points
	 *            cast=(CONST POINT *),flags=no_out critical
	 */
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native boolean PostMessageW(int /* long */hWnd, int Msg, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native boolean PostMessageA(int /* long */hWnd, int Msg, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native boolean PostThreadMessageA(int idThread, int Msg, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(HDC)
	 */
	public static final native boolean Rectangle(int /* long */hdc, int nLeftRect, int nTopRect, int nRightRect,
			int nBottomRect);
	 ...
	 *            cast=(HRGN)
	 */
	public static final native boolean RedrawWindow(int /* long */hWnd, RECT lprcUpdate, int /* long */hrgnUpdate,
			int flags);
	 ...
	 *            cast=(LPDWORD)
	 */
	public static final native int RegEnumKeyExW(int /* long */hKey, int dwIndex, char[] lpName, int[] lpcName,
			int[] lpReserved, char[] lpClass, int[] lpcClass, FILETIME lpftLastWriteTime);
	 ...
	 *            cast=(LPDWORD)
	 */
	public static final native int RegEnumKeyExA(int /* long */hKey, int dwIndex, byte[] lpName, int[] lpcName,
			int[] lpReserved, byte[] lpClass, int[] lpcClass, FILETIME lpftLastWriteTime);
	 ...
	 *            cast=(PHKEY)
	 */
	public static final native int RegOpenKeyExW(int /* long */hKey, char[] lpSubKey, int ulOptions, int samDesired,
			int /* long */[] phkResult);
	 ...
	 *            cast=(PHKEY)
	 */
	public static final native int RegOpenKeyExA(int /* long */hKey, byte[] lpSubKey, int ulOptions, int samDesired,
			int /* long */[] phkResult);
	 ...
	 *            cast=(LPDWORD)
	 */
	public static final native int RegQueryValueExW(int /* long */hKey, char[] lpValueName, int /* long */lpReserved,
			int[] lpType, char[] lpData, int[] lpcbData);
	 ...
	 *            cast=(LPDWORD)
	 */
	public static final native int RegQueryValueExA(int /* long */hKey, byte[] lpValueName, int /* long */lpReserved,
			int[] lpType, byte[] lpData, int[] lpcbData);
	...
	/**
	 * @param psds
	 *            cast=(const SCRIPT_DIGITSUBSTITUTE*)
	 * @param psc
	 *            cast=(SCRIPT_CONTROL*)
	 * @param pss
	 *            cast=(SCRIPT_STATE*)
	 */
	public static final native int ScriptApplyDigitSubstitution(SCRIPT_DIGITSUBSTITUTE psds, SCRIPT_CONTROL psc,
			SCRIPT_STATE pss);
	 ...
	 *            cast=(int *)
	 */
	public static final native int ScriptCPtoX(int iCP, boolean fTrailing, int cChars, int cGlyphs,
			int /* long */pwLogClust, int /* long */psva, int /* long */piAdvance, SCRIPT_ANALYSIS psa, int[] piX);
	 ...
	 *            cast=(SCRIPT_CACHE *)
	 * @param sfp
	 *            cast=(SCRIPT_FONTPROPERTIES *)
	 */
	public static final native int ScriptGetFontProperties(int /* long */hdc, int /* long */psc,
	 ...
	 *            cast=(int *)
	 */
	public static final native int ScriptGetLogicalWidths(SCRIPT_ANALYSIS psa, int cChars, int cGlyphs,
			int /* long */piGlyphWidth, int /* long */pwLogClust, int /* long */psva, int[] piDx);

	/**
	 * @param pwcInChars
	 *            cast=(const WCHAR *)
	 * @param psControl
	 *            cast=(const SCRIPT_CONTROL *)
	 * @param psState
	 *            cast=(const SCRIPT_STATE *)
	 * @param pItems
	 *            cast=(SCRIPT_ITEM *)
	 * @param pcItems
	 *            cast=(int *)
	 */
	public static final native int ScriptItemize(char[] pwcInChars, int cInChars, int cMaxItems,
			SCRIPT_CONTROL psControl, SCRIPT_STATE psState, int /* long */pItems, int[] pcItems);
	 ...
	 *            cast=(int *)
	 */
	public static final native int ScriptJustify(int /* long */psva, int /* long */piAdvance, int cGlyphs, int iDx,
			int iMinKashida, int /* long */piJustify);

	/**
	 * @param pbLevel
	 *            cast=(const BYTE *)
	 * @param piVisualToLogical
	 *            cast=(int *)
	 * @param piLogicalToVisual
	 *            cast=(int *)
	 */
	public static final native int ScriptLayout(int cRuns, byte[] pbLevel, int[] piVisualToLogical,
	 ...
	 *            cast=(ABC *)
	 */
	public static final native int ScriptPlace(int /* long */hdc, int /* long */psc, int /* long */pwGlyphs,
			int cGlyphs, int /* long */psva, SCRIPT_ANALYSIS psa, int /* long */piAdvance, int /* long */pGoffset,
			int[] pABC);
	 ...
	 *            cast=(WORD*)
	 */
	public static final native int ScriptGetCMap(int /* long */hdc, int /* long */psc, char[] pwcChars, int cChars,
			int dwFlags, short[] pwOutGlyphs);
	 ...
	 *            cast=(int *)
	 */
	public static final native int ScriptShape(int /* long */hdc, int /* long */psc, char[] pwcChars, int cChars,
			int cMaxGlyphs, SCRIPT_ANALYSIS psa, int /* long */pwOutGlyphs, int /* long */pwLogClust, int /* long */psva,
			int[] pcGlyphs);
	 ...
	 *            cast=(const BYTE*)
	 * @param pssa
	 *            cast=(SCRIPT_STRING_ANALYSIS*)
	 */
	public static final native int ScriptStringAnalyse(int /* long */hdc, char[] pString, int cString, int cGlyphs,
			int iCharset, int dwFlags, int iReqWidth, SCRIPT_CONTROL psControl, SCRIPT_STATE psState, int /* long */piDx,
			int /* long */pTabdef, int /* long */pbInClass, int /* long */pssa);

	/**
	 * @param ssa
	 *            cast=(SCRIPT_STRING_ANALYSIS*),flags=struct
	 */
	public static final native int ScriptStringOut(int /* long */ssa, int iX, int iY, int uOptions, RECT prc,
			int iMinSel, int iMaxSel, boolean fDisabled);
	 ...
	 *            cast=(const GOFFSET *)
	 */
	public static final native int ScriptTextOut(int /* long */hdc, int /* long */psc, int x, int y, int fuOptions,
			RECT lprc, SCRIPT_ANALYSIS psa, int /* long */pwcReserved, int iReserved, int /* long */pwGlyphs,
			int cGlyphs, int /* long */piAdvance, int /* long */piJustify, int /* long */pGoffset);
	 ...
	 *            cast=(int *)
	 */
	public static final native int ScriptXtoCP(int iX, int cChars, int cGlyphs, int /* long */pwLogClust,
			int /* long */psva, int /* long */piAdvance, SCRIPT_ANALYSIS psa, int[] piCP, int[] piTrailing);
	 ...
	 *            cast=(HRGN)
	 */
	public static final native int ScrollWindowEx(int /* long */hWnd, int dx, int dy, RECT prcScroll, RECT prcClip,
			int /* long */hrgnUpdate, RECT prcUpdate, int flags);
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native int /* long */SendMessageW(int /* long */hWnd, int Msg, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(LPARAM)
	 */
	public static final native int /* long */SendMessageA(int /* long */hWnd, int Msg, int /* long */wParam,
			int /* long */lParam);
	 ...
	 *            cast=(HDC)
	 * @param pColors
	 *            cast=(RGBQUAD *),flags=no_out critical
	 */
	 ...
	 *            cast=(HWND)
	 */
	public static final native boolean SetWindowPos(int /* long */hWnd, int /* long */hWndInsertAfter, int X, int Y,
	 ...
	 *            cast=(HINSTANCE)
	 */
	public static final native int /* long */SetWindowsHookExW(int idHook, int /* long */lpfn, int /* long */hMod,
			int dwThreadId);
	 ...
	 *            cast=(HINSTANCE)
	 */
	public static final native int /* long */SetWindowsHookExA(int idHook, int /* long */lpfn, int /* long */hMod,
			int dwThreadId);
	 ...
	 *            cast=(SHFILEINFOW *)
	 */
	public static final native int /* long */SHGetFileInfoW(char[] pszPath, int dwFileAttributes, SHFILEINFOW psfi,
	 ...
	 *            cast=(SHFILEINFOA *)
	 */
	public static final native int /* long */SHGetFileInfoA(byte[] pszPath, int dwFileAttributes, SHFILEINFOA psfi,
			int cbFileInfo, int uFlags);

	/**
	 * @param lpExecInfo
	 *            cast=(LPSHELLEXECUTEINFOW)
	 */
	 ...
	 *            cast=(HDC)
	 */
	public static final native boolean StretchBlt(int /* long */hdcDest, int nXOriginDest, int nYOriginDest,
			int nWidthDest, int nHeightDest, int /* long */hdcSrc, int nXOriginSrc, int nYOriginSrc, int nWidthSrc,
			int nHeightSrc, int dwRop);
	 ...
	 *            cast=(HTREEITEM)
	 */
	public static final native boolean TreeView_GetItemRect(int /* long */hwndTV, int /* long */hitem, RECT prc,
			boolean fItemRect);
	...
	public static final native int VtblCall(int fnNumber, int /* long */ppVtbl, char[] arg0, int arg1, int arg2,
			int[] arg3, int[] arg4);
	...
	/**
	 * @param lpWideCharStr
	 *            cast=(LPCWSTR),flags=no_out critical
	 * @param lpMultiByteStr
	 *            cast=(LPSTR),flags=no_in critical
	 * @param lpDefaultChar
	 *            cast=(LPCSTR)
	 * @param lpUsedDefaultChar
	 *            cast=(LPBOOL)
	 */
	public static final native int WideCharToMultiByte(int CodePage, int dwFlags, char[] lpWideCharStr,
			int cchWideChar, byte[] lpMultiByteStr, int cchMultiByte, byte[] lpDefaultChar, boolean[] lpUsedDefaultChar);
	 ...
	 */
	public static final native int WideCharToMultiByte(int CodePage, int dwFlags, char[] lpWideCharStr,
			int cchWideChar, int /* long */lpMultiByteStr, int cchMultiByte, byte[] lpDefaultChar,

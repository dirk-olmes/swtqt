...
public class Library {
	...
	/* 64-bit support */
	static/* final */boolean IS_64 = 0x1FFFFFFFFL == 0x1FFFFFFFFL;
	...
	public static void loadLibrary(String name, boolean mapName) {
		...
		if (path != null) {
			...
			if (mapName && load(path + SEPARATOR + mappedName2)) {
				return;
			}
		}
		...
		/*
		 * Try loading library from the tmp directory if swt library path is not
		 * specified
		 */
		...
		if (path == null) {
			...
			if (mapName && load(path + SEPARATOR + fileName2)) {
				return;
			}
		}
		...
		throw new UnsatisfiedLinkError(
				"no " + libName1 + " or " + libName2 + " in swt.library.path, java.library.path or the jar file"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	}

	static String mapLibraryName(String libName) {
		/*
		 * SWT libraries in the Macintosh use the extension .jnilib but the some
		 * VMs map to .dylib.
		 */

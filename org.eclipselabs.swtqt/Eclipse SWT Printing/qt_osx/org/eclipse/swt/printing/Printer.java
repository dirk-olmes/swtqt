...
public final class Printer extends Device {
	...
	protected void create(DeviceData deviceData) {
		...
		try {
			...
			printInfo.setOrientation(data.orientation == PrinterData.LANDSCAPE ? OS.NSLandscapeOrientation
					: OS.NSPortraitOrientation);
			...
			/*
			 * Bug in Cocoa. For some reason, the output still goes to the
			 * printer when the user chooses the preview button. The fix is to
			 * reset the job disposition.
			 */
	...
	public Rectangle getBounds() {
		...
		try {
			...
			float scaling = scalingFactor();
			Point dpi = getDPI(), screenDPI = getIndependentDPI();
			return new Rectangle(0, 0, (int) (size.width * dpi.x / screenDPI.x / scaling), (int) (size.height * dpi.y
					/ screenDPI.y / scaling));
		} finally {
			if (pool != null) {
				pool.release();
	...
	public Rectangle getClientArea() {
		...
		try {
			...
			Point dpi = getDPI(), screenDPI = getIndependentDPI();
			return new Rectangle(0, 0, (int) (rect.width * dpi.x / screenDPI.x / scaling), (int) (rect.height * dpi.y
					/ screenDPI.y / scaling));
		} finally {
			if (pool != null) {
				pool.release();
	 ...
	 * 
	 * @see
	 * org.eclipse.swt.graphics.Device#internal_dispose_GC(com.trolltech.qt.
	 * gui.QPaintDeviceInterface, org.eclipse.swt.graphics.GCData)
	 */
	@Override

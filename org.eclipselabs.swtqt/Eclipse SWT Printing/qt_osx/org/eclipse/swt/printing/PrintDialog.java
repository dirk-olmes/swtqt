...
public class PrintDialog extends Dialog {
	...
	public PrinterData open() {
		...
		printInfo.setOrientation(printerData.orientation == PrinterData.LANDSCAPE ? OS.NSLandscapeOrientation
		...
		if ((getStyle() & SWT.SHEET) != 0) {
			...
			response = /* 64 */panel.runModalWithPrintInfo(printInfo);
		}
		if (response != OS.NSCancelButton) {
			...
			data.scope = new NSNumber(dict.objectForKey(OS.NSPrintAllPages)).intValue() != 0 ? PrinterData.ALL_PAGES
			...
			data.orientation = printInfo.orientation() == OS.NSLandscapeOrientation ? PrinterData.LANDSCAPE
					: PrinterData.PORTRAIT;
			NSData nsData = NSKeyedArchiver.archivedDataWithRootObject(printInfo);
			data.otherData = new byte[/* 64 */nsData.length()];
	...
	void panelDidEnd_returnCode_contextInfo(int /* long */id, int /* long */sel, int /* long */alert,
			int /* long */returnCode, int /* long */contextInfo) {
		this.returnCode = /* 64 */returnCode;
		NSApplication application = NSApplication.sharedApplication();
